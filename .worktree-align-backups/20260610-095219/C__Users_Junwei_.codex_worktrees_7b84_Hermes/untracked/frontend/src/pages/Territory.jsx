...
const TERRITORY_CACHE_VERSION = 'global-owner-territory-cache-v76-component-route-render';
...
const TERRITORY_CACHED_RENDER_PREVIEW_MAX_ACTIVE_REGIONS_PER_OWNER = 4;
...
const TERRITORY_GLOBAL_INITIAL_RENDER_RADIUS_METERS = 280_000;
...
const LAND_MASK_MIN_HOLE_AREA_SQUARE_METERS = 20_000;
const LAND_MASK_MIN_HOLE_PERIMETER_METERS = 700;
...
function initialGlobalTerritoryRenderPolygons(ownerPolygons) {
  ...
  const nearbyRivals = rivalEntries.filter((entry) => entry.distanceMeters <= TERRITORY_GLOBAL_INITIAL_RENDER_RADIUS_METERS);
  const selectedRivals = (nearbyRivals.length > 0 ? nearbyRivals : rivalEntries)
    .slice(0, Math.max(0, TERRITORY_GLOBAL_INITIAL_RENDER_MAX_OWNERS - activeEntries.length));
...
function routeTraceSegments(poly, renderGrid = {}, options = {}) {
  ...
  return (Array.isArray(poly?.routeTraces) ? poly.routeTraces : []).flatMap((trace) => {
...
function maskComponentBounds(component) {
  ...
  const gridXs = validTiles.map((tile) => tile.gridX);
  const gridYs = validTiles.map((tile) => tile.gridY);
  return {
    minX: Math.min(...gridXs),
    maxX: Math.max(...gridXs),
    minY: Math.min(...gridYs),
    maxY: Math.max(...gridYs),
  };
}
...
function consistentMaskTiles(poly, renderGrid, concreteTiles) {
  ...
  (Array.isArray(concreteTiles) ? concreteTiles : []).forEach((tile) => {
    ...
    if (tileIsNearRouteSegments(tile, candidateSegments, interiorDistanceMeters, tileMeters)) return;
...
function repairMergedOwnerMaskTiles(tiles, renderGrid = {}) {
  ...
  // Source masks are route-repaired before ownership resolution. After owner union,
  // only repair seams between already-owned tiles; do not close bays again or open
  // Central/Prospect park routes can become fake filled territory.
  return sealDenseMaskVoids(
    sealInternalMaskCorridors(
      bridgeNearbyMaskComponents(validTiles, renderGrid),
      renderGrid,
    ),
    renderGrid,
  );
...
function visualMaskRegions(tiles, options = {}) {
  return visibleMaskConnectedComponents(maskTileConnectedComponents(tiles)).flatMap((component) => {
    const componentRegions = maskBoundaryLoops(component, options)
      .filter((loop) => loop.length >= 4);
    return visibleMaskLandRegions(componentRegions, options);
  });
}
...
function visibleMaskContourRegions(exactRegions, options = {}) {
  ...
  return (Array.isArray(exactRegions) ? exactRegions : [])
    .filter((loop) => loop.length >= LAND_MASK_MIN_VISIBLE_CONTOUR_POINTS)
    .filter((loop) => loop.hasSharedBoundary
...
function visibleMaskLandGroupLoops(regions, options = {}) {
  ...
  return measuredRegions
    .filter((entry) => {
      ...
      return entry.area >= LAND_MASK_MIN_HOLE_AREA_SQUARE_METERS
        || entry.perimeter >= LAND_MASK_MIN_HOLE_PERIMETER_METERS;
    })
...
function selectDiverseMaskRegionGroups(entries, maxGroups = LAND_MASK_MAX_VISIBLE_REGIONS_PER_OWNER, options = {}) {
  const limit = Math.max(1, Math.floor(Number(maxGroups) || 1));
...
function visibleMaskLandRegionGroups(regionGroups, options = {}) {
  const entries = (Array.isArray(regionGroups) ? regionGroups : [])
    ...
    .filter(Boolean);
  const maxGroups = Number.isFinite(Number(options?.maxGroups))
...
function limitMaskRegionGroupsByLoopBudget(regionGroups, maxLoops = LAND_MASK_MAX_VISIBLE_REGIONS_PER_OWNER) {
  const loopBudget = Math.max(1, Math.floor(Number(maxLoops) || 1));
  const limitedGroups = [];
  let usedLoops = 0;
...
function outerMaskContourRegions(regionGroups, options = {}) {
  ...
  const rankedRegionGroups = (Array.isArray(regionGroups) ? regionGroups : [])
    .map((regions) => {
      ...
      const maxArea = Math.max(...measuredRegions.map((entry) => entry.area));
      const maxPerimeter = Math.max(...measuredRegions.map((entry) => entry.perimeter));
  ...
  return rankedRegionGroups
    .flatMap(({ outerRegions }) => visibleMaskContourRegions(outerRegions, { ...options, cosLat }))
    .slice(0, LAND_MASK_MAX_VISIBLE_REGIONS_PER_OWNER);
}
...
function cachedPreviewRenderEntries(entries) {
  return (Array.isArray(entries) ? entries : []).map((entry) => {
    const maxRegions = entry.active
      ? TERRITORY_CACHED_RENDER_PREVIEW_MAX_ACTIVE_REGIONS_PER_OWNER
      : TERRITORY_CACHED_RENDER_PREVIEW_MAX_RIVAL_REGIONS_PER_OWNER;
    const previewLandRegions = previewRegionSet(entry.landRegions, { maxRegions });
    const previewContourRegions = previewRegionSet(entry.contourRegions, { maxRegions });
    return {
      ...entry,
...
function territoryRecentOwnerFocusCoords(entry) {
  ...
  const hasDistinctTraceTimes = traceRecencies.length > 1
    && (Math.max(...traceRecencies) - Math.min(...traceRecencies)) > 60_000;
  ...
  if (recentTrace) {
    ...
    const localRegions = traceCenter
      ...
      : [];
    const localCoords = localRegions.flatMap((regionInfo) => regionInfo.coords);
...
function territoryOwnerFocusBoundsCoords(entries) {
  const coords = (Array.isArray(entries) ? entries : []).flatMap(territoryRecentOwnerFocusCoords);
...
function TerritoryMap({
  ...
  useEffect(() => {
    ...
    async function paintPolygons() {
      ...
      const shouldUseProcessedRenderCache = !hasRawPolygons;
      ...
      if (
        ...
        renderEntries.forEach(({ poly, tiles }, renderIndex) => {
          ...
          const componentRecords = visibleMaskConnectedComponents(maskTileConnectedComponents(tiles))
            .map((component) => {
          ...
          const regionOptions = {
            ...
            maxGroups: useLocalViewportRegionBudget
              ? LAND_MASK_MAX_LOCAL_VIEW_REGIONS_PER_OWNER
              : LAND_MASK_MAX_VISIBLE_REGIONS_PER_OWNER,
          };
          const concreteLandRegionBudget = useLocalViewportRegionBudget
            ? LAND_MASK_MAX_LOCAL_VIEW_REGIONS_PER_OWNER
            : LAND_MASK_MAX_VISIBLE_REGIONS_PER_OWNER;
...
export default function Territory() {
  ...
  const [territoryScope, setTerritoryScope] = useState('own');
