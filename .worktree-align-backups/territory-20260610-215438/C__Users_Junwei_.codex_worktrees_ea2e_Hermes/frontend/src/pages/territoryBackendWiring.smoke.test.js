...
assert.match(
  source,
  /const TERRITORY_CACHE_VERSION = 'global-owner-territory-cache-v82-mask-v21-response-16m';/,
...
assert.match(
  ...
  /private static final String POLYGON_CACHE_VERSION = "land-mask-union-v42-mask-v21-response-16m-real-user-global";/,
...
assert.match(
  source,
  /const TERRITORY_CACHED_RENDER_PREVIEW_MAX_POINTS_PER_OWNER = 5200;[\s\S]*?const TERRITORY_CACHED_RENDER_PREVIEW_MAX_ACTIVE_REGIONS_PER_OWNER = 4;[\s\S]*?const TERRITORY_CACHED_RENDER_PREVIEW_MAX_RIVAL_REGIONS_PER_OWNER = 12;[\s\S]*?const TERRITORY_CACHED_RENDER_PREVIEW_TOLERANCE_METERS = 24;/,
  'Territory cached preview render geometry should have hard point and active/rival region caps so repeat loads stay fast without preserving broken chips.',
);
...
assert.match(
  source,
  /function previewRegionSet\(regions, options = \{\}\)[\s\S]*?const maxRegions = Math\.max\(1, Math\.floor\(Number\(options\?\.maxRegions\) \|\| 1\)\);[\s\S]*?selectCachedPreviewRegions\(simplifiedRegions[\s\S]*?\), maxRegions\);[\s\S]*?let remainingPointBudget = TERRITORY_CACHED_RENDER_PREVIEW_MAX_POINTS_PER_OWNER;[\s\S]*?function cachedPreviewRenderEntries\(entries\)[\s\S]*?const maxRegions = entry\.active[\s\S]*?TERRITORY_CACHED_RENDER_PREVIEW_MAX_ACTIVE_REGIONS_PER_OWNER[\s\S]*?TERRITORY_CACHED_RENDER_PREVIEW_MAX_RIVAL_REGIONS_PER_OWNER[\s\S]*?previewRegionSet\(entry\.landRegions, \{ maxRegions \}\)[\s\S]*?previewRegionSet\(entry\.contourRegions, \{ maxRegions \}\)/,
  'Territory cached preview should pass active/rival landmass caps into fill and contour preview generation.',
);
...
assert.match(
  source,
  /const LAND_MASK_MAX_VISIBLE_REGIONS_PER_OWNER = 32;[\s\S]*?const LAND_MASK_GLOBAL_ACTIVE_VISIBLE_REGIONS_PER_OWNER = 12;[\s\S]*?const LAND_MASK_MAX_LOCAL_VIEW_REGIONS_PER_OWNER = 144;[\s\S]*?const shouldUseViewportRegionPriority = Boolean\(selectedOwnerKeyValue\);[\s\S]*?const useLocalViewportRegionBudget = shouldUseViewportRegionPriority[\s\S]*?&& viewportMovedAfterFitRef\.current[\s\S]*?&& lastFittedConcreteBoundsKeyRef\.current !== null[\s\S]*?&& Number\(renderViewport\?\.zoom\) > 12;[\s\S]*?maxGroups: !selectedOwnerKeyValue && active[\s\S]*?LAND_MASK_GLOBAL_ACTIVE_VISIBLE_REGIONS_PER_OWNER[\s\S]*?useLocalViewportRegionBudget[\s\S]*?LAND_MASK_MAX_LOCAL_VIEW_REGIONS_PER_OWNER[\s\S]*?const concreteLandRegionBudget = !selectedOwnerKeyValue && active[\s\S]*?LAND_MASK_GLOBAL_ACTIVE_VISIBLE_REGIONS_PER_OWNER[\s\S]*?useLocalViewportRegionBudget[\s\S]*?LAND_MASK_MAX_LOCAL_VIEW_REGIONS_PER_OWNER[\s\S]*?limitMaskRegionGroupsByLoopBudget\([\s\S]*?visibleMaskLandRegionGroups\(componentRecords, regionOptions\),[\s\S]*?concreteLandRegionBudget[\s\S]*?\)/,
  'Territory should cap global active islands for readability while preserving the larger selected-owner local park override without making Global zoom reselect loops.',
);
...
assert.match(
  source,
  /function territoryRecentOwnerFocusCoords\(entry\)[\s\S]*?territoryBoundsSpanMeters\(allBounds\) <= TERRITORY_MAX_AUTO_FIT_SPAN_METERS[\s\S]*?const traceInfos = \(Array\.isArray\(entry\?\.routeTraces\)[\s\S]*?territoryTraceRecencyScore\(trace, index\)[\s\S]*?const hasDistinctTraceTimes = traceRecencies\.length > 1[\s\S]*?> 60_000;[\s\S]*?const recentTrace = traceInfos[\s\S]*?hasDistinctTraceTimes[\s\S]*?b\.recency - a\.recency[\s\S]*?territoryTraceFocusScore\(b\) - territoryTraceFocusScore\(a\)[\s\S]*?territoryCenterDistanceMeters\(regionInfo\.center, traceCenter\) <= TERRITORY_MAX_AUTO_FIT_SPAN_METERS[\s\S]*?return localCoords\.length > 0 \? localCoords : recentTrace\.coords[\s\S]*?function territoryOwnerFocusBoundsCoords\(entries\)[\s\S]*?flatMap\(territoryRecentOwnerFocusCoords\)/,
...
assert.match(
  territoryServiceSource,
  /Map<String, Map<OwnerActivityKey, MaskCellClaim>> claimsByCell[\s\S]*?OwnerActivityKey sourceKey = new OwnerActivityKey\(row\.getUserId\(\), row\.getActivityId\(\)\);[\s\S]*?computeIfAbsent\(sourceKey, ignored -> new MaskCellClaim\(row\.getUserId\(\), point\.x\(\), point\.y\(\)\)\)/,
  'Territory backend should keep same-owner cell claims split by activity so open park runs do not inherit unrelated owner-wide geometry.',
);
...
assert.match(
  ...
  'Territory should preserve each backend activity mask as a render source while grouping UI color and focus by owner.',
);
...
assert.match(
  source,
  /const LAND_MASK_TOPOLOGY_CLOSE_RADIUS_RATIO = 3\.0;[\s\S]*?const LAND_MASK_TOPOLOGY_CLOSE_MAX_RADIUS_CELLS = 12;[\s\S]*?const LAND_MASK_TOPOLOGY_CLOSE_MAX_DILATION_OPS = 120_000;[\s\S]*?const LAND_MASK_TOPOLOGY_LARGE_CLOSE_MIN_AREA_SQUARE_METERS = 5_000_000;[\s\S]*?const LAND_MASK_TOPOLOGY_LARGE_CLOSE_MIN_TILES = 30_000;[\s\S]*?const LAND_MASK_TOPOLOGY_LARGE_CLOSE_RADIUS_RATIO = 3\.0;[\s\S]*?const LAND_MASK_TOPOLOGY_LARGE_CLOSE_MAX_DILATION_OPS = 80_000;[\s\S]*?function closeThinMaskBays\(tiles, renderGrid = \{\}, options = \{\}\)[\s\S]*?const largeLandmass = Boolean\(options\?\.largeLandmass\);[\s\S]*?const maxDilationOps = largeLandmass[\s\S]*?LAND_MASK_TOPOLOGY_LARGE_CLOSE_MAX_DILATION_OPS[\s\S]*?LAND_MASK_TOPOLOGY_CLOSE_MAX_DILATION_OPS[\s\S]*?if \(validTiles\.length \* offsets\.length > maxDilationOps\) \{[\s\S]*?return validTiles;[\s\S]*?\}[\s\S]*?dilatedKeys\.add\(`\$\{tile\.gridY \+ offsetY\}:\$\{tile\.gridX \+ offsetX\}`\)[\s\S]*?if \(!closesBay\) return;[\s\S]*?function repairConsistentMaskTiles\(tiles, renderGrid = \{\}, options = \{\}\)[\s\S]*?return closeThinMaskBays\([\s\S]*?options,[\s\S]*?\);[\s\S]*?const largeLandmass = \([\s\S]*?areaSquareMeters >= LAND_MASK_TOPOLOGY_LARGE_CLOSE_MIN_AREA_SQUARE_METERS[\s\S]*?concreteTiles\.length >= LAND_MASK_TOPOLOGY_LARGE_CLOSE_MIN_TILES/,
...
assert.match(
  source,
  /const LAND_MASK_LARGE_BAY_COLLAPSE_WIDTH_RATIO = 16\.0;[\s\S]*?function collapseLargeMaskBays\(points, options = \{\}\)[\s\S]*?LAND_MASK_LARGE_BAY_COLLAPSE_MIN_ARC_TO_CHORD[\s\S]*?open\.slice\(startIndex, endIndex \+ 1\)\.some\(\(point\) => point\?\.hasSharedBoundary\)[\s\S]*?maskArcAreaMetersSquared[\s\S]*?const bayCollapsed = options\?\.largeLandmass[\s\S]*?collapseLargeMaskBays\(open, \{ cosLat, sourceCellMeters: baySourceCellMeters \}\)[\s\S]*?const componentRecords = visibleMaskConnectedComponents\(maskTileConnectedComponents\(tiles\)\)[\s\S]*?options: \{ largeLandmass, routeCorridor \}[\s\S]*?visibleMaskLandRegionGroups\(componentRecords, regionOptions\)/,
...
assert.match(
  source,
  /function visibleMaskConnectedComponents\(components\)[\s\S]*?const visibleComponents = validComponents\.filter[\s\S]*?LAND_MASK_MIN_VISIBLE_COMPONENT_TILES[\s\S]*?return validComponents[\s\S]*?\.sort\(\(a, b\) => b\.length - a\.length\)[\s\S]*?function visualMaskRegions\(tiles, options = \{\}\)[\s\S]*?visibleMaskConnectedComponents\(maskTileConnectedComponents\(tiles\)\)\.flatMap\(\(component\) => \{[\s\S]*?const componentRegions = maskBoundaryLoops\(component, options\)[\s\S]*?visibleMaskLandRegions\(componentRegions, options\)/,
  'Territory should trace visible cohesive concrete components without promoting every tiny post-conflict crumb or collapsing distant same-owner claims into one world-spanning shape.',
);
...
assert.match(
  source,
  /const LAND_MASK_MIN_CONTOUR_AREA_SQUARE_METERS = 4_000;[\s\S]*?const LAND_MASK_MIN_CONTOUR_PERIMETER_METERS = 260;[\s\S]*?const LAND_MASK_MIN_HOLE_AREA_SQUARE_METERS = 20_000;[\s\S]*?const LAND_MASK_MIN_HOLE_PERIMETER_METERS = 700;[\s\S]*?const LAND_MASK_MIN_VISIBLE_COMPONENT_TILES = 10;[\s\S]*?const LAND_MASK_EDGE_COMPONENT_MIN_VISIBLE_TILES = 4;/,
...
assert.match(
  source,
  /function visibleMaskLandGroupLoops\(regions, options = \{\}\)[\s\S]*?dominantRegion[\s\S]*?const isOuter = entry\.loop === dominantRegion\?\.loop;[\s\S]*?LAND_MASK_MIN_HOLE_AREA_SQUARE_METERS[\s\S]*?LAND_MASK_MIN_HOLE_PERIMETER_METERS[\s\S]*?visibleMaskLandRegionGroups\(regionGroups, options = \{\}\)[\s\S]*?const groupOptions = source\?\.options[\s\S]*?visibleMaskContourRegions\(visibleMaskLandGroupLoops\(regions, groupOptions\), groupOptions\)/,
  'Territory should prune tiny interior hole artifacts while preserving dominant outer territory loops.',
);
...
assert.match(
  source,
  /const LAND_MASK_MAX_VISIBLE_REGIONS_PER_OWNER = 32;[\s\S]*?function maskLoopSignedAreaMetersSquared\(loop, cosLat\)[\s\S]*?function outerMaskContourRegions\(regionGroups, options = \{\}\)[\s\S]*?signedArea: maskLoopSignedAreaMetersSquared\(loop, cosLat\),[\s\S]*?const maxArea = Math\.max\(\.\.\.measuredRegions\.map\(\(entry\) => entry\.area\)\);[\s\S]*?const maxPerimeter = Math\.max\(\.\.\.measuredRegions\.map\(\(entry\) => entry\.perimeter\)\);[\s\S]*?const dominantRegion = measuredRegions[\s\S]*?const dominantOrientation = Math\.sign\(dominantRegion\?\.signedArea \|\| 0\);[\s\S]*?orientation === dominantOrientation[\s\S]*?\.sort\(\(a, b\) => \([\s\S]*?b\.score - a\.score[\s\S]*?\|\| Number\(b\.hasSharedBoundary\) - Number\(a\.hasSharedBoundary\)[\s\S]*?\)\)[\s\S]*?slice\(0, LAND_MASK_MAX_VISIBLE_REGIONS_PER_OWNER\)[\s\S]*?visibleMaskContourRegions\(outerRegions, \{ \.\.\.options, cosLat \}\)[\s\S]*?slice\(0, LAND_MASK_MAX_VISIBLE_REGIONS_PER_OWNER\)/,
...
assert.match(
  source,
  /const sourceRenderEntries = resolveMaskTileOwnership\(renderPolygons, renderGrid\)\.slice\(\)\.reverse\(\);[\s\S]*?const renderEntries = mergeResolvedMaskEntriesByOwner\(sourceRenderEntries, renderGrid\);[\s\S]*?const globalOccupied = new Set\(renderEntries\.flatMap[\s\S]*?const componentRecords = visibleMaskConnectedComponents\(maskTileConnectedComponents\(tiles\)\)[\s\S]*?const exactRegionGroups = componentRecords\.map\(\(record\) => record\.regions\);[\s\S]*?const concreteLandRegionGroups = limitMaskRegionGroupsByLoopBudget\([\s\S]*?visibleMaskLandRegionGroups\(componentRecords, regionOptions\),[\s\S]*?\);[\s\S]*?const concreteLandRegions = visibleMaskStrokeRegions\(concreteLandRegionGroups\.flat\(\), \{ cosLat \}\);[\s\S]*?landRegions: concreteLandRegions,[\s\S]*?landRegionGroups: concreteLandRegionGroups,[\s\S]*?contourRegions: concreteContourRegions,/,
...
assert.match(
  source,
  /const globalOccupied = new Set\(renderEntries\.flatMap[\s\S]*?maskTileClaimKey\(tile\)[\s\S]*?maskBoundaryLoops\(component, \{ globalOccupied \}\)[\s\S]*?const regionOptions = \{[\s\S]*?globalOccupied,[\s\S]*?\};[\s\S]*?const concreteLandRegionGroups = limitMaskRegionGroupsByLoopBudget\([\s\S]*?visibleMaskLandRegionGroups\(componentRecords, regionOptions\),[\s\S]*?\)/,
...
assert.match(
  territoryCss,
  /\.territory-map-only \.terr-scope-switcher \{[\s\S]*?display: grid !important;[\s\S]*?grid-template-columns: repeat\(2, minmax\(172px, 216px\)\);[\s\S]*?\.territory-map-only \.terr-scope-button \{[\s\S]*?--terr-theme-color[\s\S]*?\.territory-map-only \.terr-scope-button\.is-selected \{/,
  'Territory CSS should render a compact two-button territory scope switcher with selected state.',
);
...
assert.match(
  ...
  /proof\.dom\.activeConcreteStyle\?\.fillRule === "evenodd"[\s\S]*?active concrete fill should use evenodd[\s\S]*?proof\.dom\.activeConcreteMoveCommandCount <= 32/,
  'Live shared-account proof should require evenodd active fills so route-corridor holes are preserved while excessive broken pieces are still rejected.',
);

assert.match(
  liveSharedVerifierSource,
  /const activeConcreteSmallSubpathCount = activeConcreteSubpathAreas[\s\S]*?Math\.abs\(area\) < 1800[\s\S]*?proof\.dom\.activeConcreteMoveCommandCount <= 32[\s\S]*?proof\.dom\.activeConcreteSmallSubpathCount <= 12/,
  'Live shared-account proof should fail when active territory regresses into many tiny broken subpaths.',
);
