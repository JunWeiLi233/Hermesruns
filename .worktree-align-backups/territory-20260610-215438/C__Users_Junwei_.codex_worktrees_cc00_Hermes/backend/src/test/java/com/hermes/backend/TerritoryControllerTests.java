...
class TerritoryControllerTests {
    ...
    void polygonsEndpointReturnsOneConcreteUnionMaskInsteadOfPerRunBlocks() throws Exception {
        ...
        mockMvc.perform(get("/api/territory/polygons")
                ...
                .andExpect(jsonPath("$.polygons[0].shapeType").value("land-mask"))
                .andExpect(jsonPath("$.polygons[0].cells").isArray())
                .andExpect(jsonPath("$.polygons[0].routeTraces.length()").value(2))
                .andExpect(jsonPath("$.polygons[0].routeTraces[*].points").isArray());
    }
    ...
    void polygonsEndpointCapsMergedRouteTracePayloadForLargeTerritories() throws Exception {
        ...
        JsonNode activePolygon = objectMapper.readTree(result.getResponse().getContentAsString())
                .path("polygons")
                .get(0);
        JsonNode routeTraces = activePolygon.path("routeTraces");
        assertThat(routeTraces).hasSizeLessThanOrEqualTo(256);

        int routePointCount = 0;
        for (JsonNode trace : routeTraces) {
    ...
    void polygonsEndpointAddsAllSameOwnerRunCoverageWithoutCreatingHullBetweenRuns() throws Exception {
        ...
        double ribbonBaseLat = baseLat + 0.009;
        double ribbonLengthMeters = 520.0;
        double ribbonSeparationMeters = 110.0;
        double ribbonLengthLng = ribbonLengthMeters / (TerritoryPolygonComputer.METERS_PER_DEG_LAT * cosLat);
        double ribbonSeparationLat = ribbonSeparationMeters / TerritoryPolygonComputer.METERS_PER_DEG_LAT;
        List<double[]> runBackRibbon = new ArrayList<>();
        addRouteSegment(runBackRibbon, ribbonBaseLat, baseLng, ribbonBaseLat, baseLng + ribbonLengthLng, 54);
        addRouteSegment(runBackRibbon, ribbonBaseLat, baseLng + ribbonLengthLng,
                ribbonBaseLat + ribbonSeparationLat, baseLng + ribbonLengthLng, 14);
        addRouteSegment(runBackRibbon, ribbonBaseLat + ribbonSeparationLat, baseLng + ribbonLengthLng,
                ribbonBaseLat + ribbonSeparationLat, baseLng, 54);
        seedRoute(ribbonActivity, runBackRibbon);
        ...
        MvcResult result = mockMvc.perform(get("/api/territory/polygons")
                ...
                .andExpect(jsonPath("$.polygons[1].shapeType").value("land-mask"))
                .andReturn();
        ...
        for (JsonNode polygon : root.path("polygons")) {
            ...
            assertThat(polygon.path("routeTraces")).hasSize(1);
        }

        assertThat(containsNearbyMaskCell(cells, baseLat + sideLat / 2.0, baseLng + sideLng / 2.0, 18.0)).isTrue();
        assertThat(containsNearbyMaskCell(cells, ribbonBaseLat, baseLng + ribbonLengthLng / 2.0, 18.0)).isTrue();
        assertThat(containsNearbyMaskCell(cells,
                ribbonBaseLat + ribbonSeparationLat,
                baseLng + ribbonLengthLng / 2.0,
                18.0)).isTrue();
        assertThat(containsNearbyMaskCell(cells,
                ribbonBaseLat + ribbonSeparationLat / 2.0,
                baseLng + ribbonLengthLng / 2.0,
                18.0)).isFalse();
    ...
    void polygonsEndpointKeepsRecentRunCorridorsWithoutFillingUnrunGaps() throws Exception {
        ...
        double baseLng = -122.250;
        double cosLat = Math.cos(Math.toRadians(baseLat));
        double eastThirtySixMeters = 36.0 / (TerritoryPolygonComputer.METERS_PER_DEG_LAT * cosLat);
        double eastEighteenMeters = 18.0 / (TerritoryPolygonComputer.METERS_PER_DEG_LAT * cosLat);
        ...
        seedCompactOutAndBackRoute(eastRun, baseLat, baseLng + eastThirtySixMeters);
        ...
        assertThat(containsNearbyMaskCell(cells, midRouteLat, baseLng, 12.0)).isTrue();
        assertThat(containsNearbyMaskCell(cells, midRouteLat, baseLng + eastThirtySixMeters, 12.0)).isTrue();
        assertThat(containsNearbyMaskCell(cells, baseLat + 0.010 + 8 * 0.00008, baseLng, 12.0)).isTrue();
        assertThat(containsNearbyMaskCell(cells, midRouteLat, baseLng + eastEighteenMeters, 10.0)).isFalse();
    }
