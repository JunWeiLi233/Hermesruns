...
try {
  ...
  await Promise.all([
    page.waitForURL((url) => !/\/login\b/.test(String(url)), { timeout: 15_000 }).catch(() => null),
    page.click("button[type=\"submit\"]"),
  ]);

  const alertTexts = await page.locator("[role=\"alert\"]").allTextContents().catch(() => []);
  assert(!/\/login\b/.test(page.url()), `Shared-account login did not leave /login. Alerts: ${alertTexts.join(" | ")}`);

  await page.goto(new URL("/territory", baseUrl).toString(), { waitUntil: "domcontentloaded", timeout: 20_000 });
  await page.waitForFunction(() => {
    return document.querySelectorAll(".terr-land-mask-concrete-land--active").length > 0
      && document.querySelectorAll(".terr-land-mask-contour--active").length > 0;
  }, { timeout: 20_000 });
  ...
  assert(proof.activeBackendCells > 0, "Territory polygons API returned no active backend mask cells.");
  ...
  assert(
    proof.dom.activeConcreteStyle?.fillRule === "evenodd",
    `Live /territory active concrete fill should use evenodd so route-corridor holes remain unfilled: ${proof.dom.activeConcreteStyle?.fillRule}`,
  );
  assert(
    proof.dom.activeConcreteMoveCommandCount <= 32,
    `Live /territory active concrete fill is broken into too many pieces: M commands=${proof.dom.activeConcreteMoveCommandCount}`,
  );
  assert(
    proof.dom.activeConcreteSmallSubpathCount <= 12,
    `Live /territory active concrete fill contains too many tiny broken pieces: small subpaths=${proof.dom.activeConcreteSmallSubpathCount}`,
  );
