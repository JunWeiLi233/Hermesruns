...
assert.match(
  territorySource,
  /MAP_CHROME_COPY[\s\S]*gameHud:[\s\S]*localBattle:[\s\S]*playerTerritory:[\s\S]*ownedSectors:[\s\S]*loadingKicker:[\s\S]*sectorValue:/,
  'Territory game HUD, loading screen, and personal territory dock should use bilingual local copy instead of hardcoded text.',
);
...
assert.match(
  territorySource,
  /className="terr-game-hud"[\s\S]*className="terr-game-territory-dock"/,
  'Territory should keep the website HUD and dock model without the old campaign overlay.',
);
