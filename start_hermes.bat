...
echo ==========================================================
:: 中文警告原文（以下命令向用户输出相同内容，用 [char] 码点以保持本文件为纯 ASCII）：
:: 警告：端口 8080 当前空闲，但上次提供服务的代码目录（树）不是本目录，且未被干净停止（runtime.json 标记未清理）。从此目录启动会把应用切换到另一套代码与界面设计。已中止本次启动。如确需切换：（a）在另一个目录运行 stop_hermes.bat（会删除标记），（b）手动删除 C:\Users\Junwei\.hermes\runtime.json，或（c）设置环境变量 HERMES_ALLOW_CROSS_TREE_START=1 后重试。
