...
import com.hermes.backend.races.model.SegmentIntersection;
...
public class RaceCourseMapGeometryService {
    ...
    public AlignmentPlausibilityVerdict assessAlignmentPlausibility(
        ...
        if (distanceKm != null && distanceKm > 0) {
            ...
            AlignmentRatioWindow ratioWindow = new AlignmentRatioWindow(0.30, 3.0);
            ...
            if (minimumRoutePoints >= MIN_ALIGNMENT_ROUTE_POINTS) {
                AlignmentRatioWindow expectedWindow = expectedDistanceRatioWindow(distanceKm, routePoints.size());
        ...
        RouteGeometryDiagnosis diagnosis = diagnoseRouteGeometry(routePoints, raceType, distanceKm);
    ...
    public AlignmentRatioWindow expectedDistanceRatioWindow(Double distanceKm, int routePointCount) {
        if (distanceKm != null && distanceKm >= 40.0) {
            if (routePointCount >= 18) {
                return new AlignmentRatioWindow(0.78, 1.22);
            }
            if (routePointCount >= 14) {
                return new AlignmentRatioWindow(0.65, 1.35);
            }
            return new AlignmentRatioWindow(0.55, 1.45);
        }
        if (routePointCount >= 16) {
            return new AlignmentRatioWindow(0.60, 1.40);
        }
        return new AlignmentRatioWindow(0.45, 1.70);
    }
    ...
    public List<RoutePoint> processRoutePoints(List<RoutePoint> routePoints, PromptRaceType raceType) {
        ...
        if (raceType == PromptRaceType.OUT_AND_BACK) {
            return sanitizeRoutePoints(rebuildOutAndBackRoute(repaired));
        }
        return sanitizeRoutePoints(repaired);
    }

    public List<RoutePoint> repairSelfIntersections(List<RoutePoint> routePoints, PromptRaceType raceType) {
        ...
        while (pass < SELF_INTERSECTION_REPAIR_LIMIT) {
            SegmentIntersection intersection = findFirstSelfIntersection(repaired);
    ...
    private SegmentIntersection findFirstSelfIntersection(List<RoutePoint> routePoints) {
        for (int firstSegmentEnd = 1; firstSegmentEnd < routePoints.size(); firstSegmentEnd++) {
            for (int secondSegmentEnd = firstSegmentEnd + 2; secondSegmentEnd < routePoints.size(); secondSegmentEnd++) {
                ...
                if (segmentsIntersectProperly(
                    ...
                    return new SegmentIntersection(firstSegmentEnd, secondSegmentEnd);
                }
            }
        }
        return null;
    ...
    private List<RoutePoint> collapseIntersection(List<RoutePoint> routePoints, SegmentIntersection intersection) {
    ...
    public RouteGeometryDiagnosis diagnoseRouteGeometry(List<RoutePoint> routePoints, PromptRaceType raceType, Double distanceKm) {
        ...
        } else if (startDistanceBacktracks >= 2 && raceType == PromptRaceType.POINT_TO_POINT) {
        ...
        return new RouteGeometryDiagnosis(
                selfIntersections,
                allowedSelfIntersections,
    ...
    public int allowedSelfIntersections(PromptRaceType raceType) {
    ...
    public int countStartDistanceBacktracks(List<RoutePoint> routePoints, PromptRaceType raceType, Double distanceKm) {
        if (raceType != PromptRaceType.POINT_TO_POINT || routePoints == null || routePoints.size() < 3) {
            return 0;
        }
