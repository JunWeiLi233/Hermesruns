...
public class RaceCourseMapAiService {
    ...
    public CourseMapAlignment analyzeCandidate(
    ...
    public CourseMapAlignment analyzeCandidate(
        ...
        CourseMapAlignment alignment = requestAlignmentPrepared(
                preparedImageBytes,
                preparedMediaType,
                promptText,
                latitude,
        ...
        RouteGeometryDiagnosis diagnosis = geometryService.diagnoseRouteGeometry(alignment.routePoints(), raceType, distanceKm);
        ...
        CourseMapAlignment corrected = requestAlignmentPrepared(
                preparedImageBytes,
                preparedMediaType,
     ...
     */
    private CourseMapAlignment requestAlignmentPrepared(
            byte[] preparedBytes,
        ...
        CourseMapAlignment result = parseAlignment(text, latitude, longitude, distanceKm, raceType);
        if (result == null) {
    ...
    private CourseMapAlignment parseAlignment(String text, Double latitude, Double longitude, Double distanceKm, PromptRaceType raceType) {
        try {
            String json = extractJsonObject(text);
            if (json == null) return null;
            Map<String, Object> parsed = objectMapper.readValue(json, new TypeReference<>() {});
            List<CourseMapAlignment> alignments = new ArrayList<>();
            CourseMapAlignment rootAlignment = parseAlignmentCandidate(parsed, false, raceType);
            ...
            if (rawCandidates instanceof List<?> candidateList) {
                for (Object candidate : candidateList) {
                    CourseMapAlignment parsedCandidate = parseAlignmentCandidate(candidate, true, raceType);
            ...
            CourseMapAlignment best = chooseBestAlignment(alignments, latitude, longitude, distanceKm, raceType);
    ...
    private CourseMapAlignment parseAlignmentCandidate(Object raw, boolean defaultIsCourseMap, PromptRaceType raceType) {
    ...
    public double scoreAlignmentCandidate(
        ...
        RouteGeometryDiagnosis diagnosis = geometryService.diagnoseRouteGeometry(routePoints, raceType, distanceKm);
