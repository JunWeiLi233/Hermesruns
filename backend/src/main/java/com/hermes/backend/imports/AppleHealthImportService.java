...
import com.hermes.backend.coaching.DailyHRVData;
import com.hermes.backend.coaching.DailyHRVDataRepository;
import com.hermes.backend.coaching.DailySleepData;
import com.hermes.backend.coaching.DailySleepDataRepository;
import com.hermes.backend.coaching.DailyStressData;
import com.hermes.backend.coaching.DailyStressDataRepository;
import com.hermes.backend.coaching.DailyWellnessSummary;
...
public class AppleHealthImportService {
    ...
    private static final Logger log = LoggerFactory.getLogger(AppleHealthImportService.class);
    ...
    private final ExecutorService executor = Executors.newFixedThreadPool(
            2,
            r -> {
                Thread t = new Thread(r, "apple-health-import");
                t.setDaemon(true);
                return t;
            }
    );

    public AppleHealthImportService(
    ...
    public boolean importWellnessData(Runner runner, List<Map<String, Object>> dataPoints) {
        ...
        executor.submit(() -> {
            try {
                ...
                tracker.markCompleted("Apple Health data points processed.");
            } catch (Exception e) {
                log.error("Apple Health import failed for runner {}: {}", runner.getId(), e.getMessage());
    ...
    private void processDataPoints(Runner runner, List<Map<String, Object>> dataPoints, HealthSyncTracker tracker) {
        ImportProvider provider = ImportProvider.APPLE_HEALTH;
        int savedCount = 0;
        ...
        for (Map<String, Object> entry : dataPoints) {
            try {
                ...
                if ("wellness".equals(type)) {
                    ...
                    if (entry.containsKey("deepMinutes")) entity.setDeepSleepSeconds(intVal(entry.get("deepMinutes")) * 60);
                    if (entry.containsKey("remMinutes")) entity.setRemSleepSeconds(intVal(entry.get("remMinutes")) * 60);
                ...
                log.warn("Skipping Apple Health entry for runner {}: {}", runner.getId(), e.getMessage());
            }
        }
