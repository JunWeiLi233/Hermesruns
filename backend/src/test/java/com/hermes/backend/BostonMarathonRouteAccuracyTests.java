...
class BostonMarathonRouteAccuracyTests {
    ...
    private static final Path ROUTE_FILE = Paths.get(
            "..",
            "backend",
            "course-map-images",
            "routes",
            "boston-marathon-successful-route.json"
    );
    ...
    void bostonMarathonRouteFollowsTheRealGeographicCorridor() throws Exception {
        JsonNode root = new ObjectMapper().readTree(Files.readAllBytes(resolveRouteFile()));
    ...
    private static Path resolveRouteFile() {
        // Backend Maven test working dir is `backend/`, so the relative path
        // back up + into the route file resolves correctly. If a future CI
        // runner uses a different working dir, fall back to absolute search.
        if (Files.exists(ROUTE_FILE)) return ROUTE_FILE;
        Path absolute = Paths.get(System.getProperty("user.dir"))
                .resolve("course-map-images")
                .resolve("routes")
                .resolve("boston-marathon-successful-route.json");
        return absolute;
    }
