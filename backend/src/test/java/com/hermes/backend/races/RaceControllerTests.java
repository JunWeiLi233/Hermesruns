...
class RaceControllerTests {
    ...
    void savedStatusReturnsSavedRaceSummaryWhenRaceExists() {
        ...
        RaceController controller = createController(
                authService,
                raceEventRepository,
                activityRepository,
        ...
        assertThat(response.getBody()).isInstanceOf(SavedRaceStatusResponse.class);
        SavedRaceStatusResponse payload = (SavedRaceStatusResponse) response.getBody();
    ...
    void createReturnsSavedRaceResponseForValidRequest() {
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).isInstanceOf(RaceEventResponse.class);
        RaceEventResponse payload = (RaceEventResponse) response.getBody();
    ...
    private RaceController createController(AuthService authService, RaceEventRepository races,
                                            ActivityRepository activities, RaceOfficialImageService images,
                                            RaceElevationProfileService elevation, RaceCourseMapService courseMaps) {
        return createController(authService, races, activities, images, elevation, courseMaps, null);
    }

    private RaceController createController(AuthService authService, RaceEventRepository races,
                                            ActivityRepository activities, RaceOfficialImageService images,
                                            RaceElevationProfileService elevation, RaceCourseMapService courseMaps,
                                            AutomatedCoachService coach) {
        return new RaceController(authService, new RaceEventService(races, activities, coach),
                images, elevation, courseMaps);
    }
    ...
    private static final class RaceRequestBuilder {
        ...
        private RaceEventRequest build() {
            return new RaceEventRequest(
                    name,
                    organization,
                    location,
                    eventDate,
