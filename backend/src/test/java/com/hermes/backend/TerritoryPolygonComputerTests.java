...
class TerritoryPolygonComputerTests {
    ...
    void landMaskClosedRouteFillsLoopInterior() {
        ...
        assertThat(mask.isLandTerritory()).isTrue();
        assertThat(mask.kind()).isEqualTo(TerritoryPolygonComputer.TerritoryMaskKind.LAND);
    ...
    @Test
    void landMaskSparseGeneratedClosedOutlineIsDismissed() {
        ...
        // This is a closed outline, but it is not concrete GPS coverage from a real run.
        addSegment(route, baseLat, baseLng, baseLat, baseLng + widthLng, 8);
        addSegment(route, baseLat, baseLng + widthLng, baseLat + heightLat, baseLng + widthLng, 8);
        addSegment(route, baseLat + heightLat, baseLng + widthLng, baseLat + heightLat, baseLng, 8);
        addSegment(route, baseLat + heightLat, baseLng, baseLat, baseLng, 8);

        assertThat(route).hasSizeLessThan(48);
        assertThat(computer.detectTerritoryMasks(route)).isEmpty();
    }

    @Test
    void landMaskEndpointClosedRouteFillsInteriorWhenGapIsExactGpsNoise() {
        double baseLat = 37.822;
        double baseLng = -122.25;
        double sideMeters = 260.0;
        double gapMeters = 4.0;
        double latStep = sideMeters / TerritoryPolygonComputer.METERS_PER_DEG_LAT;
    ...
    @Test
    void landMaskNearEndpointGapIsDismissed() {
        double baseLat = 37.822;
        double baseLng = -122.25;
        double sideMeters = 260.0;
    ...
    @Test
    void landMaskEndpointGapAboveGpsNoiseIsDismissed() {
        double baseLat = 37.822;
        double baseLng = -122.25;
        double sideMeters = 260.0;
        double gapMeters = 18.0;
        double latStep = sideMeters / TerritoryPolygonComputer.METERS_PER_DEG_LAT;
        double gapLat = gapMeters / TerritoryPolygonComputer.METERS_PER_DEG_LAT;
        double lngStep = latStep / Math.cos(Math.toRadians(baseLat));
        List<double[]> route = new ArrayList<>();
    ...
    @Test
    void landMaskOpenRouteIsDismissed() {
        List<double[]> route = new ArrayList<>();
        double baseLat = 37.822;
        double baseLng = -122.25;
        for (int i = 0; i <= 80; i++) {
    ...
    @Test
    void landMaskOpenRouteDoesNotCreateWideTerritoryBlock() {
        List<double[]> route = new ArrayList<>();
        double baseLat = 37.822;
        double baseLng = -122.25;
        for (int i = 0; i <= 80; i++) {
    ...
    @Test
    void encodedCurrentLandMaskRoundTripsAsLandTerritory() {
        double baseLat = 37.822;
        double baseLng = -122.25;
        double sideMeters = 240.0;
        double latStep = sideMeters / TerritoryPolygonComputer.METERS_PER_DEG_LAT;
        ...
        TerritoryPolygonComputer.DetectedTerritoryMask routeMask = computer.detectTerritoryMasks(route).get(0);
        String encoded = TerritoryPolygonComputer.encodeMaskCells(
                routeMask.cells(),
                routeMask.cellMeters(),
                routeMask.kind()
        );
        TerritoryPolygonComputer.DecodedTerritoryMask decoded = TerritoryPolygonComputer.decodeMaskCells(encoded);

        assertThat(decoded.processed()).isTrue();
        assertThat(decoded.isLandTerritory()).isTrue();
        assertThat(decoded.cells()).isNotEmpty();
    }

    @Test
    void landMaskSparseStraightRouteIsDismissed() {
        List<double[]> route = new ArrayList<>();
        double baseLat = 37.822;
        double baseLng = -122.25;
    ...
    @Test
    void landMaskLongStraightLineIsDismissed() {
        List<double[]> route = new ArrayList<>();
        double baseLat = 37.822;
        double baseLng = -122.25;
        double cosLat = Math.cos(Math.toRadians(baseLat));
    ...
    @Test
    void landMaskLargeNearClosedLoopWithParkScaleEndpointGapIsDismissed() {
        double baseLat = 37.822;
        double baseLng = -122.25;
    ...
    @Test
    void landMaskOpenCentralParkLengthRouteWithParkScaleEndpointGapIsDismissed() {
    ...
    @Test
    void landMaskOpenLargeURouteIsDismissed() {
        double baseLat = 37.822;
        double baseLng = -122.25;
        double widthMeters = 1_300.0;
        double heightMeters = 900.0;
    ...
    @Test
    void landMaskOpenCShapeIsDismissed() {
        double baseLat = 37.822;
        double baseLng = -122.25;
    ...
    @Test
    void landMaskOpenReturnPathIsDismissed() {
        double baseLat = 37.822;
        double baseLng = -122.25;
    ...
    @Test
    void landMaskNarrowOpenReturnPathIsDismissed() {
        double baseLat = 37.822;
        double baseLng = -122.25;
    ...
    @Test
    void landMaskMidRouteGapWithTailsIsDismissed() {
        double baseLat = 37.822;
        double baseLng = -122.25;
        double sideMeters = 260.0;
    ...
    @Test
    void landMaskMidRouteNearMissIsDismissed() {
        double baseLat = 37.822;
        double baseLng = -122.25;
        double sideMeters = 260.0;
        double gapMeters = 10.0;
    ...
    @Test
    void landMaskLongLeadInEndpointClosureIsDismissed() {
        double baseLat = 40.742;
        double baseLng = -73.823;
    ...
    @Test
    void landMaskShortLeadInTailStillClaimsClosedLoopLand() {
        double baseLat = 37.822;
        double baseLng = -122.25;
        double sideMeters = 260.0;
        double tailMeters = 8.0;
        double cosLat = Math.cos(Math.toRadians(baseLat));
        double latStep = sideMeters / TerritoryPolygonComputer.METERS_PER_DEG_LAT;
        double lngStep = sideMeters / (TerritoryPolygonComputer.METERS_PER_DEG_LAT * cosLat);
    ...
    @Test
    void landMaskOriginPassWithCooldownFinishAwayIsDismissed() {
        double baseLat = 40.742;
        double baseLng = -73.823;
        double sideMeters = 620.0;
        double closePassMeters = 5.0;
        double finishGapMeters = 10.0;
        double cosLat = Math.cos(Math.toRadians(baseLat));
        double latStep = sideMeters / TerritoryPolygonComputer.METERS_PER_DEG_LAT;
        double lngStep = sideMeters / (TerritoryPolygonComputer.METERS_PER_DEG_LAT * cosLat);
        double closePassLng = closePassMeters / (TerritoryPolygonComputer.METERS_PER_DEG_LAT * cosLat);
        double finishGapLng = finishGapMeters / (TerritoryPolygonComputer.METERS_PER_DEG_LAT * cosLat);
        List<double[]> route = new ArrayList<>();

        addSegment(route, baseLat, baseLng, baseLat, baseLng + lngStep, 64);
        addSegment(route, baseLat, baseLng + lngStep, baseLat + latStep, baseLng + lngStep, 64);
        addSegment(route, baseLat + latStep, baseLng + lngStep, baseLat + latStep, baseLng, 64);
        addSegment(route, baseLat + latStep, baseLng, baseLat, baseLng + closePassLng, 64);
        addSegment(route, baseLat, baseLng + closePassLng, baseLat, baseLng + finishGapLng, 8);

        assertThat(computer.detectTerritoryMasks(route)).isEmpty();
    }

    @Test
    void landMaskEndpointClosedSkinnyOutAndBackIsDismissed() {
        double baseLat = 40.742;
        double baseLng = -73.823;
        double lengthMeters = 1_600.0;
        double separationMeters = 28.0;
        double cosLat = Math.cos(Math.toRadians(baseLat));
        double lengthLat = lengthMeters / TerritoryPolygonComputer.METERS_PER_DEG_LAT;
        double separationLng = separationMeters / (TerritoryPolygonComputer.METERS_PER_DEG_LAT * cosLat);
        List<double[]> route = new ArrayList<>();

        addSegment(route, baseLat, baseLng, baseLat + lengthLat, baseLng, 180);
        addSegment(route, baseLat + lengthLat, baseLng, baseLat + lengthLat, baseLng + separationLng, 8);
        addSegment(route, baseLat + lengthLat, baseLng + separationLng, baseLat, baseLng + separationLng, 180);
        addSegment(route, baseLat, baseLng + separationLng, baseLat, baseLng, 8);

        assertThat(computer.detectTerritoryMasks(route)).isEmpty();
    }

    @Test
    void landMaskCityBlockTailIsDismissedAfterLeavingClosedShape() {
        double baseLat = 40.742;
        double baseLng = -73.823;
        double widthMeters = 1_200.0;
        double heightMeters = 780.0;
        double tailMeters = 99.0;
        double cosLat = Math.cos(Math.toRadians(baseLat));
        double widthLng = widthMeters / (TerritoryPolygonComputer.METERS_PER_DEG_LAT * cosLat);
        double heightLat = heightMeters / TerritoryPolygonComputer.METERS_PER_DEG_LAT;
        double tailLng = tailMeters / (TerritoryPolygonComputer.METERS_PER_DEG_LAT * cosLat);
        ...
        addSegment(route, baseLat + heightLat, baseLng + widthLng, baseLat + heightLat, baseLng, 72);
        addSegment(route, baseLat + heightLat, baseLng, baseLat, baseLng, 48);
        addSegment(route, baseLat, baseLng, baseLat, baseLng - tailLng, 12);
    ...
    @Test
    void landMaskKissenaStyleNearEndpointRouteIsDismissedUntilConcreteLoopCloses() {
        List<double[]> anchors = List.of(
                new double[]{40.751094, -73.820596},
                new double[]{40.749648, -73.822282},
                new double[]{40.748648, -73.821310},
                new double[]{40.747147, -73.817142},
                new double[]{40.748817, -73.814141},
                new double[]{40.747258, -73.812430},
                new double[]{40.749531, -73.807987},
                new double[]{40.747173, -73.806273},
                new double[]{40.747494, -73.804393},
                new double[]{40.748206, -73.803917},
                new double[]{40.747992, -73.803019},
                new double[]{40.742242, -73.804560},
                new double[]{40.742109, -73.808502},
                new double[]{40.743076, -73.814332},
                new double[]{40.745688, -73.813920},
                new double[]{40.747688, -73.816089},
                new double[]{40.747294, -73.816982},
                new double[]{40.748126, -73.819675},
                new double[]{40.749972, -73.822338},
                new double[]{40.751177, -73.820548}
        );
        List<double[]> route = new ArrayList<>();
        for (int i = 1; i < anchors.size(); i += 1) {
            double[] start = anchors.get(i - 1);
            double[] end = anchors.get(i);
            addSegment(route, start[0], start[1], end[0], end[1], 8);
        }
    ...
    void staleMaskEncodingDoesNotDecodeAsLegacyPolygonCoordinates() {
        ...
        // v20 rows could store long open-route corridors on coarse adaptive grids; v23 rows
        // could preserve partial Kissena compartment fills instead of recomputing; v25 rows could
        // fill near-endpoint Kissena routes that did not concretely close; v26 rows could fill
        // origin-anchored loops whose actual finish drifted outside the close threshold; v27 rows
        // could still classify skinny endpoint-closed out-and-back traces as land; v29 rows could
        // classify sparse generated closed outlines as land.
        ...
        String previousSparseOpenRouteMask = "mask:v21:8|37.822,-122.25;37.823,-122.25";
        String previousTightTailMask = "mask:v22:8|37.822,-122.25;37.823,-122.25";
        String previousPartialKissenaMask = "mask:v23:16|37.822,-122.25;37.823,-122.25";
        String previousSolidKissenaMask = "mask:v24:16|37.822,-122.25;37.823,-122.25";
        String previousLooseEndpointLandOnlyMask = "mask:v25:16|37.822,-122.25;37.823,-122.25";
        String previousFinishDriftMask = "mask:v26:16|37.822,-122.25;37.823,-122.25";
        String previousSkinnyLoopMask = "mask:v27:16|37.822,-122.25;37.823,-122.25";
        String previousSparseGeneratedOutlineMask = "mask:v29:land:16|37.822,-122.25;37.823,-122.25";
        ...
        assertThat(TerritoryPolygonComputer.decodeMaskCells(previousSparseOpenRouteMask).cells()).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeMaskCells(previousTightTailMask).cells()).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeMaskCells(previousPartialKissenaMask).cells()).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeMaskCells(previousSolidKissenaMask).cells()).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeMaskCells(previousLooseEndpointLandOnlyMask).cells()).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeMaskCells(previousFinishDriftMask).cells()).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeMaskCells(previousSkinnyLoopMask).cells()).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeMaskCells(previousSparseGeneratedOutlineMask).cells()).isEmpty();
        ...
        assertThat(TerritoryPolygonComputer.decodeCoordinates(previousSparseOpenRouteMask)).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeCoordinates(previousTightTailMask)).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeCoordinates(previousPartialKissenaMask)).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeCoordinates(previousSolidKissenaMask)).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeCoordinates(previousLooseEndpointLandOnlyMask)).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeCoordinates(previousFinishDriftMask)).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeCoordinates(previousSkinnyLoopMask)).isEmpty();
        assertThat(TerritoryPolygonComputer.decodeCoordinates(previousSparseGeneratedOutlineMask)).isEmpty();
    }
