...
class SleepPollingTests {
    ...
    void garminWakeCatchUpCoversPersistedGapWithinBootstrapBound(int daysAgo, int expectedDays) {
        ...
        when(garmin.startWellnessImport(eq(runner), anyString(), anyString(), anyInt())).thenReturn(true);

        context.withPropertyValues("app.background.polling.enabled=false").run(ctx -> {
            ctx.getBean(GarminWellnessSyncScheduler.class).triggerAdminSync(null, "wake_catchup");
            verify(garmin).startWellnessImport(runner, "runner@example.test", "test-password", expectedDays);
        });
    }
}
