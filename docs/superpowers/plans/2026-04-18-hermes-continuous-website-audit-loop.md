...
**Tech Stack:** Node `.mjs` helpers in `tools/`, existing `.workspace/state` artifacts, `tools/auto-hermes-tools.test.mjs`, existing runtime-proof helpers, existing queue/controller/loop helpers
...
const DEFAULT_RUNS_DIR = ".workspace/state/auto-hermes-runs";
...
export function runAutoHermesWebsiteAudit({
  ...
  contextLedger = ".workspace/state/CONTEXT_LEDGER.md",
  pagesIndex = ".workspace/codex/pages.md",
  outputJson = ".workspace/state/AUTO_HERMES_WEBSITE_AUDIT.json",
  outputMd = ".workspace/state/AUTO_HERMES_WEBSITE_AUDIT.md",
  write = false,
} = {}) {
...
`, "utf8");
  ...
  assert.equal(state.websiteAudit?.candidate?.files?.includes("frontend/src/pages/analysis/Analysis.jsx"), true);
