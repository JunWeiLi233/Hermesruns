...
Run Id: auto-hermes-attack-20260420041829
Mode: attack
Status: completed
Generated: 2026-04-20T04:18:29.219Z

## Summary
Attack simulation completed. 42 total findings (6 runtime-verified). Active probes: 6 runtime-verified findings (0 CRITICAL, 3 HIGH). 
...
- [MEDIUM] pii-leak-hunter :: Sensitive Data Exposure: Entity/DTO might leak PII in JSON responses.
  ...
  Verification: static-only
- [LOW] active-rate-limit :: Login endpoint rate-limits after 1 attempts — rate limiting is active.
...
## Task Writeback
- active-data-leak:-api-config-status:config-status-endpoint-leaks-sensitive-configuration-data-without-authentication.
- active-data-leak:-api-billing-config:billing-config-endpoint-leaks-sensitive-configuration-data-without-authentication.
- active-webhook-abuse:-api-strava-webhook:strava-webhook-accepts-unauthenticated-forged-activity-events.
