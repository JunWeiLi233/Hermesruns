...
- Do not manually stop after one empty-queue observation. If `.workspace/state/AUTO_HERMES_SELF_LOOP.json` shows `supervisorState.stop: false`, `supervisorState.decision: "continue"`, and no work unit, the self-loop helper should already have re-entered; if `selfReentryLimitReached` is true, inspect the loop state before raising `--max-self-reentries`.
...
3. Read `.workspace/state/AUTO_HERMES_SELF_COORDINATOR.md`.
...
- Run `node tools/auto-hermes-self-loop.mjs --write --runtime codex --dry-run` and verify `.workspace/state/AUTO_HERMES_SELF_LOOP.json` has `selfExecutionContract: "parent-codex-native-subagents"`.
- Verify `.workspace/state/AUTO_HERMES_SELF_COORDINATOR.md` contains `Codex Self-Loop Protocol (Active Execution)`.
...
1. Read `.workspace/state/AUTO_HERMES_SELF_COORDINATOR.md` for the current work unit.
2. Read `.workspace/state/AUTO_HERMES_SELF_CONTROLLER.json` for the subagent plan, route, knowledge pack, files, and verification contract.
...
2. Read `.workspace/state/AUTO_HERMES_SELF_COORDINATOR.md`.
