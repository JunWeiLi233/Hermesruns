...
try {
  ...
  await primePage.waitForFunction(() => document.querySelector(".terr-leaflet-map"), { timeout: 20_000 });
  const initialPolygonProof = await primePage.evaluate(async () => {
    const token = localStorage.getItem("hermes_jwt");
    const response = await fetch("/api/territory/polygons?initial=true", {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    }).catch((error) => ({ ok: false, status: "fetch-error", error: String(error) }));
    if (!response?.json) {
      return response;
    }
    const data = await response.json().catch(() => null);
    return {
      ...
      declaredPolygonCount: Number(data?.polygonCount || 0),
      backfillInProgress: Boolean(data?.backfillInProgress),
      pendingActivityCount: Number(data?.pendingActivityCount || 0),
    };
  });
  if (
    initialPolygonProof.ok
    && initialPolygonProof.polygonCount === 0
    && initialPolygonProof.declaredPolygonCount === 0
    && !initialPolygonProof.backfillInProgress
    && initialPolygonProof.pendingActivityCount === 0
  ) {
    console.log(JSON.stringify({
      ok: true,
      email: sharedEmail,
      noRenderableTerritory: true,
      proof: initialPolygonProof,
    }, null, 2));
