...
<!-- Runtime: opencode; command: /auto-hermes-market; contract: docs/ai/EDITING_CONTRACT.md -->
...
2. Create `.workspace/state/market/` directory if absent
...
|---|---|---|
| Market Analyst | `.workspace/state/market/market-analyst.json` | `"<scope>" market size 2024`, `"<scope>" TAM billion` |
| Competitor Hunter | `.workspace/state/market/competitor-hunter.json` | `best <scope> apps 2024`, `<scope> alternatives site:reddit.com` |
| Pricing Engineer | `.workspace/state/market/pricing-engineer.json` | `<competitor>/pricing` for each verified competitor |
| Social Signal | `.workspace/state/market/social-signal.json` | `site:reddit.com "<scope>"`, `"<scope>" "worth paying for"` |
| Trend Validator | `.workspace/state/market/trend-validator.json` | `"<scope>" funding 2024 site:techcrunch.com`, `"<scope>" trend growing` |
| SEO Agent (optional) | `.workspace/state/market/seo-agent.json` | `best <scope>`, `<scope> alternatives`, `<scope> vs`, `<scope> guide` |
...
- Use the Ralph executor for parallel agent launch when the core market agents are independent. The SEO lane should be added only when the scope is consumer-searchable and should still keep its write scope isolated to `.workspace/state/market/seo-agent.json`.
