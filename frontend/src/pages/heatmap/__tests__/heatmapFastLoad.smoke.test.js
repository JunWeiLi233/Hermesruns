...
const profileHeatmapSource = readFileSync(
  path.join(here, "../../../../../backend/src/main/java/com/hermes/backend/runner/ProfileHeatmapService.java"),
  'utf8',
);
...
const backendMaxPageLimitMatch = profileHeatmapSource.match(/MAX_HEATMAP_PAGE_LIMIT\s*=\s*(\d+)\s*;/);
assert.ok(
  backendMaxPageLimitMatch,
  'ProfileHeatmapService should declare MAX_HEATMAP_PAGE_LIMIT so the frontend sample size can be cross-checked.',
);
const backendSampleLimitMatch = profileHeatmapSource.match(/DEFAULT_HEATMAP_SAMPLE_LIMIT\s*=\s*(\d+)\s*;/);
assert.ok(
  backendSampleLimitMatch,
  'ProfileHeatmapService should declare DEFAULT_HEATMAP_SAMPLE_LIMIT so the frontend sample size can be cross-checked.',
);
assert.equal(
...
assert.match(
  authContextSource,
  /import \{ invalidateHeatmapCache \} from '\.\.\/utils\/heatmap\/cache\.js';[\s\S]*?function handleStravaSyncFinished\(\) \{[\s\S]*?invalidateHeatmapCache\(email\);/,
