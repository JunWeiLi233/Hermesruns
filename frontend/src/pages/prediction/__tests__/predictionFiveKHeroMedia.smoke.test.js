...
const assetPath = path.join(here, "../../../assets/generated/prediction-5k-hero.webp");

assert.ok(existsSync(assetPath), 'The 5K prediction hero needs its supplied street-running image asset.');
assert.equal(existsSync(path.join(here, '../../../assets/generated/prediction-5k-hero-1200.webp')), false, 'The byte-identical full-size WebP copy should not be retained.');
assert.match(pageSource, /\$\{predictionFiveKHeroImage\} 1200w/, 'The largest responsive WebP should reuse the fallback image at its existing width.');
assert.match(
  pageSource,
