...
assert.match(
  finalStyleSource,
  /body:is\(\.theme-light,\s*\.theme-high-contrast-light,\s*\.theme-midnight,\s*\.theme-high-contrast\) #root \.analysis-page-shell \.analysis-profile-bento-grid\s*\{[^}]*border:\s*0\s*!important;[^}]*border-radius:\s*0\s*!important;[^}]*padding:\s*0\s*!important;[^}]*background:\s*transparent\s*!important;[^}]*box-shadow:\s*none\s*!important;/,
  'The final Analysis stylesheet should clear the outer bento surface after shared card rules.',
);

assert.match(
  finalStyleSource,
  /body:is\(\.theme-light,\s*\.theme-high-contrast-light,\s*\.theme-midnight,\s*\.theme-high-contrast\) #root \.analysis-page-shell \.analysis-profile-bento-grid\s*>\s*\.analysis-profile-bento-card\s*\{[^}]*border:\s*0\s*!important;[^}]*border-radius:\s*var\(--ahs-radius-lg\)\s*!important;/,
...
assert.match(
  finalStyleSource,
  /body:is\(\.theme-light,\s*\.theme-high-contrast-light,\s*\.theme-midnight,\s*\.theme-high-contrast\) #root \.analysis-page-shell \.analysis-profile-reference-card\.is-trend \.analysis-overview-card-head\s*\{[^}]*display:\s*grid\s*!important;[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s+auto\s*!important;[^}]*grid-template-rows:\s*auto\s+auto\s*!important;/,
  'Trend card header should provide a right-side stack for direction and delta.',
);

assert.match(
  finalStyleSource,
  /body:is\(\.theme-light,\s*\.theme-high-contrast-light,\s*\.theme-midnight,\s*\.theme-high-contrast\) #root \.analysis-page-shell \.analysis-profile-reference-card\.is-trend \.analysis-overview-vdot-trend-heading\s*\{[^}]*position:\s*static\s*!important;[^}]*grid-column:\s*2\s*!important;[^}]*grid-row:\s*1\s*!important;/,
  'Trend direction should sit above the delta value.',
);

assert.match(
  finalStyleSource,
  /body:is\(\.theme-light,\s*\.theme-high-contrast-light,\s*\.theme-midnight,\s*\.theme-high-contrast\) #root \.analysis-page-shell \.analysis-profile-reference-card\.is-trend \.analysis-overview-insight-delta\s*\{[^}]*grid-column:\s*2\s*!important;[^}]*grid-row:\s*2\s*!important;/,
