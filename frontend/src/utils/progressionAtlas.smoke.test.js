...
assert.equal(atlas.chartPoints.length, 3, 'Total view should keep one point per real daily bucket when runs land on different days.');
...
assert.equal(atlas.latestPoint?.key, atlas.chartPoints[2]?.key);
assert.equal(
