...
const retiredSplitToolPath = path.join(root, 'tools/split-styles.mjs');
...
assert.equal(existsSync(retiredSplitToolPath), false, 'The deprecated forwarding alias must not return; use the canonical generator.');
...
assert.doesNotMatch(generator, /readFileSync\([^\n]*frontend\/src\/styles\/style\.css/, 'The generator must not use the frozen legacy bundle as input.');
