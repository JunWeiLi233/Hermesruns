...
const DEFAULT_LEDGER = ".workspace/state/LOCAL_CONSOLE_ERRORS.json";

const COMMON_FRONTEND_SURFACES = [
  {
    match: /frontend[\\/]+src[\\/]+pages[\\/]+shoes[\\/]+AddShoes\.jsx$/i,
    route: "/shoes/add",
    surface: "AddShoes",
    smokeTests: [
  ...
  {
    match: /frontend[\\/]+src[\\/]+pages[\\/]+today-run[\\/]+TodayRun\.jsx$/i,
    route: "/today-run",
    surface: "TodayRun",
    smokeTests: [
  ...
  {
    match: /frontend[\\/]+src[\\/]+pages[\\/]+runs[\\/]+Runs\.jsx$/i,
    route: "/runs",
    surface: "Runs",
    smokeTests: [
