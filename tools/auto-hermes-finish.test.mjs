...
function makeFixture() {
  ...
  fs.mkdirSync(path.join(dir, ".workspace/state"), { recursive: true });
  return dir;
}

{
    ...
    gitRunner: (args) => {
      ...
      if (key === "status --short --untracked-files=all") {
        return [
          ...
          "?? .workspace/cache/frontend-static-build/index.html",
          "?? .workspace/tmp/session.log",
        ].join("\n");
      }
  ...
  for (const file of ['.workspace/cache/frontend-static-build/index.html', '.workspace/tmp/session.log']) {
    assert.equal(finish.policies.find((policy) => policy.path === file)?.bucket, 'local-only');
  }
}

{
  const fixture = makeFixture();
  fs.writeFileSync(path.join(fixture, ".workspace/state", "AUTO_HERMES_DOCKER_GATE.json"), JSON.stringify({
  ...
  const finish = runAutoHermesFinish({
    ...
    dockerGateJson: path.join(fixture, ".workspace/state", "AUTO_HERMES_DOCKER_GATE.json"),
    dockerGateMd: path.join(fixture, ".workspace/state", "AUTO_HERMES_DOCKER_GATE.md"),
