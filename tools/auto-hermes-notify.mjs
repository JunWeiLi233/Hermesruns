...
function parseArgs(argv = []) {
  const args = {
    ...
    commitError: "",
    pushAttempted: false,
    outputJson: ".workspace/state/AUTO_HERMES_NOTIFY.json",
    outputMd: ".workspace/state/AUTO_HERMES_NOTIFY.md",
    notifyEnv: "",
    notifyTransport: "",
  };
...
export function runAutoHermesNotify(options = {}) {
  ...
  if (options.write) {
    fs.mkdirSync(path.dirname(resolveFromRoot(options.outputJson || ".workspace/state/AUTO_HERMES_NOTIFY.json")), { recursive: true });
    fs.writeFileSync(resolveFromRoot(options.outputJson || ".workspace/state/AUTO_HERMES_NOTIFY.json"), JSON.stringify(result, null, 2), "utf8");
    fs.writeFileSync(resolveFromRoot(options.outputMd || ".workspace/state/AUTO_HERMES_NOTIFY.md"), renderMarkdown(result), "utf8");
  }
