...
function parseArgs(rawArgs) {
  ...
  args.outputDir = String(args.outputDir || ".workspace/state/security-reports").trim() || ".workspace/state/security-reports";
...
function isIgnoredSecurityScanPath(relPath) {
  ...
  if (rel.startsWith(".workspace/codex/")) return true;
  if (rel.startsWith(".workspace/state/")) return true;
  if (rel.startsWith(".workspace/cache/")) return true;
  if (rel.startsWith(".workspace/tmp/")) return true;
...
function discoverAdminPathsFromCodex(rootDir) {
  const codexPath = path.join(rootDir, ".workspace/codex", "routes.md");
...
async function runActiveAdminEnumerationProbe(baseUrl, rootDir) {
  ...
  if (reachedAdmin.length === 0 && adminPaths.length > 0) {
    findings.push(makeFinding({
      checker: "active-admin-enumeration",
      severity: "LOW",
      summary: `Probed ${adminPaths.length} admin endpoints from .workspace/codex/routes.md — all rejected unauthenticated and forged-token requests.`,
      target: "/api/admin/*",
      evidence: [
...
export async function runAutoHermesSecurity(rawArgs = process.argv.slice(2)) {
  ...
  if (rawArgs && !Array.isArray(rawArgs) && typeof rawArgs === "object") {
    ...
    merged.outputDir = String(merged.outputDir || ".workspace/state/security-reports").trim() || ".workspace/state/security-reports";
