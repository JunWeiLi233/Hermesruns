...
function createAgentWorkflowSpec(agentLanes) {
  ...
  return {
    ...
    boxes: [
      ...
      { ...stateFiles, kind: "database", title: "Queue + Context", lines: ["TASKS.md", ".workspace/state/CONTEXT_LEDGER.md", ".workspace/state/AGENT_SYNC.md", ".workspace/state/HUMAN_LOOP.md"] },
    ...
    cards: [
      {
        ...
        items: [
          "Controller chooses one bounded round from TASKS + .workspace/state state.",
...
function createSaasArchitectureSpec(routeSummary, lazyPageNames, stackFacts = {}) {
  ...
  return {
    ...
    subtitle: "Public entry, runner shell, admin tooling, bearer-session API boundary, data stores, and third-party integrations.",
    width: 1320,
    height: 744,
    groups: [
    ...
    boxes: [
      ...
      },
      {
        ...
        kind: "security",
        title: "Auth Gates",
        lines: ["UserOnlyRoute", "AdminOnlyRoute", "Bearer token on /api"],
      },
      {
        ...backend,
        kind: "backend",
    ...
    arrows: [
      ...
      lineArrow(auth.x + auth.width, auth.centerY, backend.x, backend.y + 114, "/api + token", { labelAnchor: "end", labelDx: -9, labelDy: -2 }),
    ...
    legend: [
      ...
      { kind: "security", label: "auth gates / token boundary" },
    ...
    cards: [
      ...
      },
      {
        ...
        items: [
          ...
          "UserOnlyRoute / AdminOnlyRoute gate pages; api.ts attaches the bearer session token.",
  ...
  return {
    ...
    boxes: [
      ...
      { ...storage, kind: "external", title: "localStorage", lines: ["hermes_jwt · hermes_role", "hermes_email · hermes_lang"] },
      {
        ...
        lines: [
          ...
          "4 · JwtAuthenticationFilter — Bearer token → principal",
      ...
      { ...runnerAccess, kind: "backend", title: "Runner · authenticated", lines: ["authenticated /api/**", "activities · shoes · races", "coach · wellness · weather", "imports · profile · maps"] },
      { ...controllers, kind: "backend", title: "REST Controllers", lines: ["auth · activities · shoes", "races · coach · wellness", "imports · weather · maps"] },
      { ...services, kind: "backend", title: "Domain Services", lines: ["parse · normalize · analyze", "coaching + rating engines", "webhook processors"] },
      { ...stores, kind: "database", title: "JPA Repositories", lines: ["domain entities · H2 / PostgreSQL", "audit trail + job store", "JPA / Hibernate"] },
    ],
    arrows: [
    ...
    cards: [
      ...
      },
      {
        title: "Filter Order",
        items: [
          "Rate limits guard /api first (webhook paths exempt), then security headers, then the session-token filter.",
      ...
      },
      {
        ...
        items: [
          "REST controllers under /api delegate to domain services.",
          "JPA repositories persist domain entities on H2 (dev) or PostgreSQL (production).",
          "Webhook entry points retain their provider-specific validation and rate limits.",
        ],
      },
    ],
...
function createDataDictionarySpec() {
  ...
  const muscleSlugs = routeForBox(276, 480, 232, 76);
  ...
  return {
    ...
    subtitle: "Static frontend dictionaries, shared TypeScript contracts, and representative JPA entity families behind /api.",
    width: 1320,
    height: 850,
    groups: [
      { x: 248, y: 78, width: 288, height: 634, label: "Frontend Dictionaries — bundled", kind: "cloud" },
      { x: 792, y: 78, width: 484, height: 642, label: "Backend JPA Entity Families", kind: "cloud" },
    ],
    boxes: [
      ...
      { ...muscleSlugs, kind: "frontend", title: "muscleSlugMapper.js", lines: ["exercise name → muscle slugs", "shared strength-map matching"] },
    ...
    arrows: [
      ...
      lineArrow(coachingEntities.x, coachingEntities.centerY, muscleSlugs.x + muscleSlugs.width, muscleSlugs.centerY, "check-in slug targets", { dashed: true, labelDx: -40, labelDy: -14 }),
    ],
    legend: [
    ...
    cards: [
      {
        ...
        items: [
          "Static catalogs, locale bundles and muscle-slug mappings ship with the Vite build.",
          "i18n keeps en + zh-CN copy in lockstep and adds Accept-Language to every apiFetch.",
          "worldRaceCatalog.json is shared by the frontend and the backend course seeder.",
        ],
      },
      {
        title: "Backend Families",
        items: [
          "The diagram shows representative entity families; Runner anchors runner-owned data.",
          "Admin operations keep their own audit log, saved filters, and background job stores.",
          "Additional MFA, billing and reward entities live in their own domain packages.",
        ],
      },
      {
        title: "Correspondence",
        items: [
          "GET /api/shoe-catalog supplies live catalog data alongside the bundled fallback.",
          "contracts/*.ts type the JSON payloads (ApiErrorPayload, activity summaries, predictions).",
          "Muscle-slug helpers map exercise names to the active UI highlighter.",
        ],
      },
    ],
    footer: "Representative dictionaries and entity families; complete ownership is documented in docs/PROJECT_MAP.md.",
  };
}
