...
check("catalog helper reports repo-local VoltAgent installs and safe fallback when manifest is missing", async () => {
  ...
  const emptyCatalog = loadVoltAgentCatalog({
    rootDir: fixture.dir,
    manifestPath: ".workspace/state/AUTO_HERMES_SUBAGENT_CATALOG.json",
...
"""
`);
  writeFixtureFile(fixture.dir, ".workspace/state/AUTO_HERMES_SUBAGENT_CATALOG.json", JSON.stringify({
...
check("installer helper writes repo-local codex-only manifest and prefixes installed names", async () => {
  ...
  const fixture = makeFixture();
  writeFixtureFile(fixture.dir, ".workspace/state/voltagent-codex-subagents/categories/02-language-specialists/spring-boot-engineer.toml", `name = "spring-boot-engineer"
...
"""
`);
  writeFixtureFile(fixture.dir, ".workspace/state/voltagent-codex-subagents/categories/02-language-specialists/react-specialist.toml", `name = "react-specialist"
...
`);
  ...
  const result = syncVoltAgentCodexAgents({
    rootDir: fixture.dir,
    refreshRepo: false,
    cacheDir: ".workspace/state/voltagent-codex-subagents",
    agentsDir: ".codex/agents",
    manifestPath: ".workspace/state/AUTO_HERMES_SUBAGENT_CATALOG.json",
  });
  ...
  const manifest = JSON.parse(fs.readFileSync(path.join(fixture.dir, ".workspace/state/AUTO_HERMES_SUBAGENT_CATALOG.json"), "utf8"));
...
check("controller infers backend ownership and avoids stale human-loop surface bleed", () => {
  ...
  Files: \`backend/src/main/java/com/hermes/backend/imports/GarminConnectController.java, backend/src/test/java/com/hermes/backend/imports/GarminConnectControllerTests.java\`
...
check("controller exposes Vercel-style composition for bounded cross-stack rounds", () => {
  ...
  Files: \`frontend/src/pages/schedule/Schedule.jsx, backend/src/main/java/com/hermes/backend/routing/RoutePlannerController.java\`
...
- [ ] Fix Garmin controller trust gap
  Files: \`backend/src/main/java/com/hermes/backend/imports/GarminConnectController.java\`
  Problem: backend-logic
...
`, "utf8");
  ...
  const claimDir = path.join(fixture.dir, ".workspace/state", "claims");
  acquireTaskClaim({
    claimDir,
    key: taskClaimKey({
      source: "active-task",
...
check("controller stops cleanly when the same task is already claimed by another owner", () => {
  ...
  const { result } = runAutoHermesController({
    ...
    claimDir,
    claimOwner: "owner-b",
    outputJson: path.join(fixture.dir, ".workspace/state", "controller-b.json"),
    outputMd: path.join(fixture.dir, ".workspace/state", "controller-b.md"),
  });
...
check("loop dry-run writes Ralph grounding artifacts for the selected work unit", () => {
  ...
  const contextSnapshotDir = path.join(fixture.dir, ".workspace/state", "context-snapshots");
  const progressDir = path.join(fixture.dir, ".workspace/state", "auto-hermes-state");
  const { state } = runAutoHermesLoop({
    json: true,
    dryRun: true,
    write: true,
    ...
    coordinatorMd: path.join(fixture.dir, ".workspace/state", "coordinator.md"),
    promptFile: path.join(fixture.dir, ".workspace/state", "prompt.md"),
    loopStateJson: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_LOOP_STATE.json"),
  });
  ...
  const coordinator = fs.readFileSync(path.join(fixture.dir, ".workspace/state", "coordinator.md"), "utf8");
...
check("self-loop dry-run exposes unbounded Ralph semantics and dedicated self-loop artifacts", () => {
  ...
  const { state } = runAutoHermesSelfLoop({
    ...
    contextLedger: fixture.files.contextLedger,
    loopState: fixture.files.loopState,
    controllerJson: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_CONTROLLER.json"),
    controllerMd: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_CONTROLLER.md"),
    promotionJson: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_PROMOTION.json"),
    promotionMd: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_PROMOTION.md"),
    outputJson: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_LOOP.json"),
    outputMd: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_LOOP.md"),
    coordinatorJson: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_COORDINATOR.json"),
    coordinatorMd: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_COORDINATOR.md"),
    promptFile: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_NEXT_PROMPT.md"),
    loopStateJson: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_LOOP_STATE.json"),
    roundResultJson: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_ROUND_RESULT.json"),
    roundResultMd: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_ROUND_RESULT.md"),
  });
  ...
  const prompt = fs.readFileSync(path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_NEXT_PROMPT.md"), "utf8");
  assert.match(prompt, /keep iterating until a real stop gate fires/i);
  assert.match(prompt, /website-audit fallback/i);

  const coordinator = JSON.parse(fs.readFileSync(path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_COORDINATOR.json"), "utf8"));
...
check("self-loop resets same-task stall history when the repeated task shows new round-result progress", () => {
  ...
  const loopStateJson = path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_LOOP_STATE.json");
  const roundResultJson = path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_SELF_ROUND_RESULT.json");
  const sharedArgs = {
    json: true,
    write: true,
    runtime: "codex-live",
...
check("loop resume carries same-work-unit stall history forward for the same persisted work unit", () => {
  ...
  Verify: \`cd frontend && npm run lint && npm run build\``);
  const loopStateJson = path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_LOOP_STATE.json");
  const sharedArgs = {
    json: true,
    write: true,
    runtime: "codex-live",
...
- [ ] Fix Profile empty state
  Files: \`frontend/src/pages/profile/Profile.jsx\`
  Problem: frontend-design
  Owner: frontend-agent
...
check("codex-live loop uses executor-owned self-loop path when an executor is configured", () => {
  ...
  const coordinator = JSON.parse(fs.readFileSync(path.join(fixture.dir, ".workspace/state", "coordinator.json"), "utf8"));
...
`, "utf8");
  ...
  writeFixtureFile(fixture.dir, ".workspace/codex/pages.md", "Analysis -> frontend/src/pages/analysis/Analysis.jsx\n");
  writeFixtureFile(fixture.dir, "frontend/src/pages/analysis/Analysis.jsx", "export default function Analysis(){ return null; }\n");
  ...
  assert.deepEqual(result.files, ["frontend/src/pages/analysis/Analysis.jsx"]);
...
`, "utf8");
  ...
  const controllerJson = path.join(fixture.dir, ".workspace/state", "controller.json");
  const promptFile = path.join(fixture.dir, ".workspace/state", "prompt.md");
  const { state } = runAutoHermesLoop({
    json: true,
    dryRun: true,
    write: true,
  ...
  assert.match(prompt, /website-audit fallback/i);
  assert.match(prompt, /frontend\/src\/pages\/analysis\/Analysis\.jsx/);
});
...
`, "utf8");
  ...
  const firstPass = runAutoHermesController({
    ...
    contextLedger: fixture.files.contextLedger,
    loopState: fixture.files.loopState,
    outputJson: path.join(fixture.dir, ".workspace/state", "controller-1.json"),
    outputMd: path.join(fixture.dir, ".workspace/state", "controller-1.md"),
  }).result;
  ...
  const secondPass = runAutoHermesController({
    ...
    contextLedger: fixture.files.contextLedger,
    loopState: fixture.files.loopState,
    outputJson: path.join(fixture.dir, ".workspace/state", "controller-2.json"),
    outputMd: path.join(fixture.dir, ".workspace/state", "controller-2.md"),
  }).result;
...
`, "utf8");
  ...
  const controllerJson = path.join(fixture.dir, ".workspace/state", "controller.json");
  const controllerMd = path.join(fixture.dir, ".workspace/state", "controller.md");
...
check("finish helper includes push intent in the generated auto-commit command when requested", () => {
  const fixture = makeFixture();
  fs.writeFileSync(path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_DOCKER_GATE.json"), JSON.stringify({
    generatedAt: new Date().toISOString(),
    passed: true,
    gitHead: "",
    statusSnapshot: "",
  ...
  const finish = runAutoHermesFinish({
    ...
    files: "frontend/src/pages/profile/ProfileDashboard.jsx",
    push: true,
    dockerGateJson: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_DOCKER_GATE.json"),
    dockerGateMd: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_DOCKER_GATE.md"),
...
check("finish helper marks push-only clean stops as eligible when the current branch has unpublished commits", () => {
  const finish = runAutoHermesFinish({
    json: true,
    ...
    dockerGateStatusOverride: {
      path: ".workspace/state/AUTO_HERMES_DOCKER_GATE.json",
      present: true,
      passed: true,
      fresh: true,
...
check("finish helper executes a direct push instead of auto-commit when a clean stop only needs publication", () => {
  let commitCalls = 0;
  const pushCalls = [];
...
check("finish helper sends email notification on true-stop finish paths when SMTP and recipient config are present", () => {
  const fixture = makeFixture();
  const notifyJson = path.join(fixture.dir, ".workspace/state", "notify.json");
  const notifyMd = path.join(fixture.dir, ".workspace/state", "notify.md");
  let captured = null;
  ...
  const finish = runAutoHermesFinish({
    ...
    files: "tools/auto-hermes-loop.mjs",
    outputJson: path.join(fixture.dir, ".workspace/state", "finish.json"),
    outputMd: path.join(fixture.dir, ".workspace/state", "finish.md"),
    notifyJson,
    notifyMd,
    notifyEnv: {
      SPRING_MAIL_HOST: "smtp.example.com",
...
check("finish helper records a warning when email notification fails without blocking finish completion", () => {
  const fixture = makeFixture();
  const notifyJson = path.join(fixture.dir, ".workspace/state", "notify-warning.json");
  ...
  const finish = runAutoHermesFinish({
    ...
    outputJson: path.join(fixture.dir, ".workspace/state", "finish-warning.json"),
    outputMd: path.join(fixture.dir, ".workspace/state", "finish-warning.md"),
    notifyJson,
    notifyMd: path.join(fixture.dir, ".workspace/state", "notify-warning.md"),
    notifyEnv: {
      SPRING_MAIL_HOST: "smtp.example.com",
      AUTO_HERMES_NOTIFY_TO: "runner@example.com",
    },
...
`, "utf8");
  ...
  const { state: result } = runAutoHermesMax({
    ...
    controllerJson: path.join(fixture.dir, ".workspace/state", "controller.json"),
    controllerMd: path.join(fixture.dir, ".workspace/state", "controller.md"),
    outputJson: path.join(fixture.dir, ".workspace/state", "max.json"),
    outputMd: path.join(fixture.dir, ".workspace/state", "max.md"),
    coordinatorJson: path.join(fixture.dir, ".workspace/state", "max-coordinator.json"),
    coordinatorMd: path.join(fixture.dir, ".workspace/state", "max-coordinator.md"),
    lanesDir: path.join(fixture.dir, ".workspace/state", "lanes"),
    resultsDir: path.join(fixture.dir, ".workspace/state", "results"),
    mergeJson: path.join(fixture.dir, ".workspace/state", "merge.json"),
    mergeMd: path.join(fixture.dir, ".workspace/state", "merge.md"),
  });
  ...
  const coordinatorMd = fs.readFileSync(path.join(fixture.dir, ".workspace/state", "max-coordinator.md"), "utf8");
...
`, "utf8");
  ...
  writeFixtureFile(fixture.dir, ".workspace/codex/pages.md", "Analysis -> frontend/src/pages/analysis/Analysis.jsx\n");
  writeFixtureFile(fixture.dir, "frontend/src/pages/analysis/Analysis.jsx", "export default function Analysis(){ return null; }\n");
  const explicitPlanPath = path.join(fixture.dir, ".workspace/state", "preexisting-plan.json");
  ...
  const maxOutputJson = path.join(fixture.dir, ".workspace/state", "max.json");
  const maxOutputMd = path.join(fixture.dir, ".workspace/state", "max.md");
  const maxCoordinatorJson = path.join(fixture.dir, ".workspace/state", "max-coordinator.json");
  const maxCoordinatorMd = path.join(fixture.dir, ".workspace/state", "max-coordinator.md");
  const maxLanesDir = path.join(fixture.dir, ".workspace/state", "lanes");
  const loopOutputJson = path.join(fixture.dir, ".workspace/state", "max-loop.json");
  const loopOutputMd = path.join(fixture.dir, ".workspace/state", "max-loop.md");
  const loopCoordinatorJson = path.join(fixture.dir, ".workspace/state", "loop-coordinator.json");
  const loopCoordinatorMd = path.join(fixture.dir, ".workspace/state", "loop-coordinator.md");
  const loopLanesDir = path.join(fixture.dir, ".workspace/state", "loop-lanes");
  const loopBriefJson = path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_MAX_LOOP_BRIEF.json");
  const loopBriefMd = path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_MAX_LOOP_BRIEF.md");
  const loopPromptFile = path.join(fixture.dir, ".workspace/state", "loop-prompt.md");
  ...
  const { state: result } = runAutoHermesMax({
    ...
    liveControllerJson: path.join(fixture.dir, ".workspace/state", "controller.json"),
    liveControllerMd: path.join(fixture.dir, ".workspace/state", "controller.md"),
  ...
  assert.deepEqual(result.websiteAudit?.candidate?.files, ["frontend/src/pages/analysis/Analysis.jsx"]);
  ...
  assert.deepEqual(plannerLane.ownedFiles, ["frontend/src/pages/analysis/Analysis.jsx"]);
  ...
  const { state: loopState } = runAutoHermesMaxLoop({
    ...
    loopState: fixture.files.loopState,
    planFile: explicitPlanPath,
    liveControllerJson: path.join(fixture.dir, ".workspace/state", "controller-loop.json"),
    liveControllerMd: path.join(fixture.dir, ".workspace/state", "controller-loop.md"),
    ...
    resultsDir: path.join(fixture.dir, ".workspace/state", "loop-results"),
    mergeJson: path.join(fixture.dir, ".workspace/state", "loop-merge.json"),
    mergeMd: path.join(fixture.dir, ".workspace/state", "loop-merge.md"),
  ...
  assert.deepEqual(loopLane.ownedFiles, ["frontend/src/pages/analysis/Analysis.jsx"]);
  ...
  assert.match(loopPrompt, /frontend\/src\/pages\/analysis\/Analysis\.jsx/);
});
...
`, "utf8");
  ...
  const staleDefaultPlanPath = path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_MAX_PLAN.json");
  ...
  assert.equal(result.selectedLaneCount, 1);
  assert.equal(result.websiteAudit?.mode, "website-audit");
  assert.equal(result.websiteAudit?.candidate?.files?.[0], "frontend/src/pages/analysis/Analysis.jsx");
});
...
check("auto-hermes-max ignores a stale default no-lane plan file when the controller already has promotable work", () => {
  ...
  Files: \`backend/src/main/java/com/hermes/backend/admin/AdminController.java\`
  Problem: backend-logic
  Owner: backend-agent
...
`, "utf8");
  writeFixtureFile(fixture.dir, "PRODUCT.md", "# Product\n");
  writeFixtureFile(fixture.dir, ".workspace/codex/pages.md", "");
  ...
  const { state: loopState } = runAutoHermesMaxLoop({
    ...
    liveControllerJson: path.join(fixture.dir, ".workspace/state", "controller.json"),
    liveControllerMd: path.join(fixture.dir, ".workspace/state", "controller.md"),
    planFile: path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_MAX_PLAN.json"),
    outputJson: path.join(fixture.dir, ".workspace/state", "max-loop.json"),
    outputMd: path.join(fixture.dir, ".workspace/state", "max-loop.md"),
    briefJson: path.join(fixture.dir, ".workspace/state", "max-loop-brief.json"),
    briefMd: path.join(fixture.dir, ".workspace/state", "max-loop-brief.md"),
    coordinatorJson: path.join(fixture.dir, ".workspace/state", "max-coordinator.json"),
    coordinatorMd: path.join(fixture.dir, ".workspace/state", "max-coordinator.md"),
    lanesDir: path.join(fixture.dir, ".workspace/state", "lanes"),
    resultsDir: path.join(fixture.dir, ".workspace/state", "results"),
    mergeJson: path.join(fixture.dir, ".workspace/state", "merge.json"),
    mergeMd: path.join(fixture.dir, ".workspace/state", "merge.md"),
    promptFile: path.join(fixture.dir, ".workspace/state", "max-loop-prompt.md"),
  });
...
`, "utf8");
  ...
  fs.mkdirSync(path.join(sandboxRoot, ".workspace/state"), { recursive: true });
  ...
  fs.writeFileSync(path.join(sandboxRoot, ".workspace/state", "HUMAN_LOOP.md"), "# Human Loop\n\n## Current Status\n- Status: active\n", "utf8");
  fs.writeFileSync(path.join(sandboxRoot, ".workspace/state", "AGENT_SYNC.md"), "# Cross-Agent Sync\n\n## Active Claims\n- none\n", "utf8");
  fs.writeFileSync(path.join(sandboxRoot, ".workspace/state", "CONTEXT_LEDGER.md"), "# Context Ledger\n\n## Surface Capsules\n### Sandbox\n- Goal: Validate the real /auto-hermes-max parent loop on an isolated harness.\n", "utf8");
  fs.writeFileSync(path.join(sandboxRoot, ".workspace/state", "LOOP_STATE.md"), "# Loop State\n", "utf8");

  const planPath = path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_MAX_PLAN.json");
  ...
  const loopOutputJson = path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_MAX_LOOP.json");
  const loopOutputMd = path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_MAX_LOOP.md");
  const loopBriefJson = path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_MAX_LOOP_BRIEF.json");
  const loopBriefMd = path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_MAX_LOOP_BRIEF.md");
  const coordinatorJson = path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_MAX_COORDINATOR.json");
  const coordinatorMd = path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_MAX_COORDINATOR.md");
  const mergeJson = path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_MAX_MERGE.json");
  const mergeMd = path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_MAX_MERGE.md");
  ...
  const { state } = runAutoHermesMaxLoop({
    ...
    maxIterations: 1,
    tasks: fixture.files.tasks,
    humanLoop: path.join(sandboxRoot, ".workspace/state", "HUMAN_LOOP.md"),
    agentSync: path.join(sandboxRoot, ".workspace/state", "AGENT_SYNC.md"),
    contextLedger: path.join(sandboxRoot, ".workspace/state", "CONTEXT_LEDGER.md"),
    loopState: path.join(sandboxRoot, ".workspace/state", "LOOP_STATE.md"),
    planFile: planPath,
    liveControllerJson: path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_CONTROLLER.json"),
    liveControllerMd: path.join(sandboxRoot, ".workspace/state", "AUTO_HERMES_CONTROLLER.md"),
    outputJson: loopOutputJson,
    outputMd: loopOutputMd,
    briefJson: loopBriefJson,
    briefMd: loopBriefMd,
    coordinatorJson,
    coordinatorMd,
    lanesDir: path.join(sandboxRoot, ".workspace/state", "auto-hermes-max-lanes"),
    resultsDir,
    mergeJson,
    mergeMd,
    promptFile,
...
`, "utf8");
  ...
  const { report } = runAutoHermesWebsiteAudit({
    ...
    pagesIndex: ".workspace/codex/pages.md",
  });

  assert.equal(report.mode, "website-audit");
  assert.equal(Boolean(report.candidate), true);
  assert.equal(report.candidate.files[0], "frontend/src/pages/analysis/Analysis.jsx");
  assert.deepEqual(report.candidate.files, ["frontend/src/pages/analysis/Analysis.jsx"]);
...
`, "utf8");
  ...
  writeFixtureFile(
    fixture.dir,
    ".workspace/codex/pages.md",
    "Profile -> frontend/src/pages/profile/Profile.jsx\nAnalysis -> frontend/src/pages/analysis/Analysis.jsx\n",
  );
  writeFixtureFile(fixture.dir, "frontend/src/pages/profile/Profile.jsx", "export default function Profile(){ return null; }\n");
  writeFixtureFile(fixture.dir, "frontend/src/pages/analysis/Analysis.jsx", "export default function Analysis(){ return null; }\n");
  writeFixtureFile(fixture.dir, ".workspace/state/CONTEXT_LEDGER.md", "Profile Profile Profile Profile\nAnalysis\n");
  writeFixtureFile(fixture.dir, ".workspace/state/AGENT_SYNC.json", JSON.stringify({
    recentlyCompleted: [
      {
...
check("auto-hermes-max writes explorer markdown and parallel lanes declare parent-owned writeback with worktree isolation", () => {
  const fixture = makeFixture();
  const planPath = path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_MAX_PLAN.json");
  fs.writeFileSync(fixture.files.tasks, `# Hermes Tasks

## Active Tasks
- [ ] Cross-stack race trust fix
  Files: \`frontend/src/pages/races/RacesDetail.jsx, backend/src/main/java/com/hermes/backend/races/RaceCourseMapService.java\`
  Problem: cross-stack-contract
...
`, "utf8");
    ...
    lanes: [
      {
        ...
        ownedFiles: ["frontend/src/pages/races/RacesDetail.jsx"],
        effort: "medium",
        parallelSafe: true,
        dependsOn: [],
        dependencyMode: "parallel-ready",
      },
      {
        ...
        ownedFiles: ["backend/src/main/java/com/hermes/backend/races/RaceCourseMapService.java"],
        effort: "medium",
        parallelSafe: true,
        dependsOn: [],
        dependencyMode: "parallel-ready",
  ...
  const lanesDir = path.join(fixture.dir, ".workspace/state", "lanes");
  const resultsDir = path.join(fixture.dir, ".workspace/state", "results");
  const explorerJsonPath = path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_MAX_EXPLORER.json");
  const explorerMdPath = path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_MAX_EXPLORER.md");
  const { state: result } = runAutoHermesMax({
    json: true,
    write: true,
    runtime: "codex-live",
...
check("merge gate treats non-canonical lane statuses as blocked instead of silently accepting legacy aliases", async () => {
  const fixture = makeFixture();
  const resultFile = path.join(fixture.dir, ".workspace/state", "auto-hermes-max-results", "lane-1.json");
  fs.mkdirSync(path.dirname(resultFile), { recursive: true });
  fs.writeFileSync(resultFile, JSON.stringify({
    laneId: "lane-1",
    status: "passed",
    changedFiles: ["frontend/src/pages/analysis/Analysis.jsx"],
    ...
    lanes: [
      {
        ...
        ownedFiles: ["frontend/src/pages/analysis/Analysis.jsx"],
        dependencyState: "parallel-ready",
        isolation: "direct",
        resultFile,
        activityLogFile: path.join(fixture.dir, ".workspace/state", "auto-hermes-max-results", "lane-1-activity.json"),
      },
    ],
  };
...
`;
  ...
  const nextTask = syncQueueWithController(taskText, {
    ...
    files: [
      "backend/src/main/java/com/hermes/backend/shoes/ShoeCatalogController.java",
      "backend/src/test/java/com/hermes/backend/shoes/ShoeCatalogControllerTests.java",
    ],
    context: "Auto-suggested from codebase analysis (backend_logic_guard)",
  ...
  assert.match(nextTask, /Files: `backend\/src\/main\/java\/com\/hermes\/backend\/shoes\/ShoeCatalogController\.java, backend\/src\/test\/java\/com\/hermes\/backend\/shoes\/ShoeCatalogControllerTests\.java`/i);
...
check("trace-to-skill merges repeated trace packets into analyst sections and soft workflow candidates", async () => {
  ...
  const fixture = makeFixture();
  const roundsDir = path.join(fixture.dir, ".workspace/state", "trace-to-skill", "rounds");
  ...
  const { report } = runAutoHermesTraceToSkill({
    rootDir: fixture.dir,
    roundsDir: ".workspace/state/trace-to-skill/rounds",
    outputJson: ".workspace/state/AUTO_HERMES_TRACE_TO_SKILL.json",
    outputMd: ".workspace/state/AUTO_HERMES_TRACE_TO_SKILL.md",
  });
...
check("trace-to-skill packet writer persists round evidence and refreshes merged outputs", async () => {
  ...
  assert.equal(fs.existsSync(path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_TRACE_TO_SKILL.json")), true);
  assert.equal(fs.existsSync(path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_TRACE_TO_SKILL.md")), true);
});
...
check("controller surfaces soft trace-to-skill evidence when merged workflow signals exist", () => {
  ...
  const tracePath = path.join(fixture.dir, ".workspace/state", "AUTO_HERMES_TRACE_TO_SKILL.json");
