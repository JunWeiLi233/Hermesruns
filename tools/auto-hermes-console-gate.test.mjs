...

{
  const result = inferCommonFrontendRoutes([
    "frontend/src/pages/shoes/AddShoes.jsx",
    "frontend/src/pages/today-run/TodayRun.jsx",
    "frontend/src/pages/runs/Runs.jsx",
  ]);
  ...
  assert.deepEqual(result.smokeTests, [
    ...
    "frontend/src/pages/runs/__tests__/runsRoutePreviewCache.smoke.test.js",
  ]);
}

{
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "ah-console-"));
  const ledgerDir = path.join(rootDir, ".workspace/state");
...
{
  const plan = buildFrontendConsoleGatePlan("frontend/src/pages/shoes/AddShoes.jsx||frontend/src/pages/runs/Runs.jsx");
