...
function Get-PathPolicy {
    param(
        [Parameter(Mandatory = $true)]
        [string]$Path,
        [switch]$TrackedDeletion
    )

    $normalized = Normalize-RepoPath -Path $Path

    # Only the staged Git deletion list may enable this flag; no file contents ship.
    if ($TrackedDeletion) {
        return New-PolicyResult -Path $normalized -Bucket 'publishable' -Reason 'Removal of a previously tracked file; no local contents are published.'
    }
    if ($normalized -match '^\.workspace/(state|codex|cache|tmp)/') {
        return New-PolicyResult -Path $normalized -Bucket 'local-only' -Reason 'Workspace runtime records, checkpoints and caches must stay local.'
    }
    if ($normalized -match '^backend/src/main/resources/static/' -or $normalized -match '^\.codex/(auth\.json|config(\.local)?\.toml)$') {
        return New-PolicyResult -Path $normalized -Bucket 'should-ignore' -Reason 'Generated output or machine-local credentials/configuration must not ship.'
    }
    if ($null -eq $script:ReviewedPublishPaths) {
        $manifestPath = Join-Path $repoRoot 'tools/publishable-paths.json'
        $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
        $script:ReviewedPublishPaths = @($manifest.paths)
    }
    if ($script:ReviewedPublishPaths -contains $normalized) {
        return New-PolicyResult -Path $normalized -Bucket 'publishable' -Reason 'Explicitly reviewed repository source or portable project configuration.'
    }
...
function Get-DockerGateStatus {
    $gatePath = Join-Path $repoRoot '.workspace/state\AUTO_HERMES_DOCKER_GATE.json'
...
function Invoke-ArchitectureDiagramRefresh {
    ...
    # A repository-wide move can exceed Windows' command-line limit.
    $nodeArgs = @($refreshScript, '--json', '--force')
...
$staged = @(Invoke-Git -Args @('diff', '--cached', '--no-renames', '--name-only') | Where-Object { $_.Trim() -ne '' })
if ($staged.Count -eq 0) {
    throw 'No staged files found. Stage the intended product files first or pass -Paths.'
}

$stagedDeletions = @(Invoke-Git -Args @('diff', '--cached', '--no-renames', '--name-only', '--diff-filter=D'))
$stagedPolicies = @($staged | ForEach-Object { Get-PathPolicy -Path $_ -TrackedDeletion:($_ -in $stagedDeletions) })
