...
assert.match(toolingWorkflow, /run:\s*npm run test:tooling/);
const rootPackage = JSON.parse(readFileSync(path.resolve(rootDir, 'package.json'), 'utf8'));
assert.equal(rootPackage.scripts['test:tooling'], 'node tools/run-tool-tests.mjs');
const { discoverToolTests } = await import(pathToFileURL(path.resolve(rootDir, 'tools/run-tool-tests.mjs')).href);
assert.ok(discoverToolTests(rootDir).includes(path.resolve(rootDir, 'tools/check-functionality-direction-tree.test.mjs')), 'The CI test runner must discover the direction-tree contract.');
