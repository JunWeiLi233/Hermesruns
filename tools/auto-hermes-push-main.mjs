...
function parseArgs(argv) {
  const args = {
    ...
    draft: false,
    skipChecks: false,
    outputJson: ".workspace/state/AUTO_HERMES_PUSH_MAIN.json",
    outputMd: ".workspace/state/AUTO_HERMES_PUSH_MAIN.md",
    write: false,
  };
...
export async function runAutoHermesPushMain(rawArgs = process.argv.slice(2)) {
  ...
  try {
    ...
    if (!args.skipChecks && result.sourceCommit !== result.sourceHead) {
      markStep(result, "docker", "running");
      runCommand("node", [resolveFromRoot(args.rootDir, "tools/auto-hermes-docker-gate.mjs"), "--write"], { cwd: args.rootDir });
      markStep(result, "docker", "completed", { note: "Revalidated the final commit before pushing." });
    }
