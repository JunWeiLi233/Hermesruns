...
`);
  ...
  write("backend/src/main/java/com/hermes/backend/runner/Runner.java", `package com.hermes.backend;
...
`);
  ...
  write("backend/src/main/java/com/hermes/backend/auth/SecurityHeadersFilter.java", `package com.hermes.backend;
...
`);
  ...
  write("frontend/src/pages/auth/Login.jsx", `export default function Login() {
...
check("audit mode discovers repo targets and writes markdown/json reports", async () => {
  ...
  const { report } = await runAutoHermesSecurity({
    ...
    write: true,
    outputDir: ".workspace/state/security-reports",
    tasks: "TASKS.md",
  });
  ...
  assert.ok(report.inventory.forms.some((form) => form.file.endsWith("frontend/src/pages/auth/Login.jsx")));
  assert.ok(report.findings.some((finding) => finding.checker === "injection-hunter"));
  assert.equal(fs.existsSync(path.join(fixture.dir, ".workspace/state/security-reports", `${report.runId}.md`)), true);
  assert.equal(fs.existsSync(path.join(fixture.dir, ".workspace/state/security-reports", `${report.runId}.json`)), true);
});
...
check("secret scan skips local artifacts but still scans source files", async () => {
  ...
  write(".workspace/state/AGENT_SYNC.md", "Alex local state with sk-" + "a".repeat(40));
  write(".workspace/cache/assets/index.js", "const token = 'sk-" + "e".repeat(40) + "';");
  write(".workspace/tmp/session.log", "const token = 'sk-" + "f".repeat(40) + "';");
  ...
  assert.equal(secretFindings.some((finding) => finding.file.startsWith(".workspace/state/")), false);
  assert.equal(secretFindings.some((finding) => finding.file.startsWith(".workspace/cache/")), false);
  assert.equal(secretFindings.some((finding) => finding.file.startsWith(".workspace/tmp/")), false);
...
check("secret scan ignores source URLs and repository paths but catches assigned credentials", async () => {
  ...
  write(
    "backend/src/main/java/com/hermes/backend/auth/SecurityConfig.java",
...
`,
  ...
  write(
    ...
    JSON.stringify({
      provider: "backend/src/main/java/com/hermes/backend/auth/PasswordResetService.java",
      tests: ["frontend/src/pages/runs/__tests__/runDetailProfileCockpit.smoke.test.js"],
    }),
  );
