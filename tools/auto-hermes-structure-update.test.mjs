...
`);
  ...
  write(".workspace/state/HUMAN_LOOP.md", `## Current Status
- Status: autonomous
...
`);
  ...
  write(".workspace/state/AGENT_SYNC.md", `# Cross-Agent Sync

## Active Claims
- none
`);

  write(".workspace/state/CONTEXT_LEDGER.md", "# Context Ledger\n");
...
check("writeback prepends a dedicated structure subsection and writes the steering brief", async () => {
  ...
  const briefJson = JSON.parse(fs.readFileSync(path.join(fixture.dir, ".workspace/state/AUTO_HERMES_STRUCTURE_UPDATE.json"), "utf8"));
...
check("controller prefers the structure-update default on the next eligible round when no active work exists", async () => {
  ...
  fixture.write(".workspace/state/AUTO_HERMES_STRUCTURE_UPDATE.json", JSON.stringify({
    status: "ready",
...
- [ ] Teach auto-hermes to honor structure-update steering briefs
  Files: \`tools/auto-hermes-controller.mjs\`, \`.workspace/state/AUTO_HERMES_STRUCTURE_UPDATE.json\`
...
`);
  ...
  const { result } = runAutoHermesController({
    ...
    humanLoop: path.join(fixture.dir, ".workspace/state/HUMAN_LOOP.md"),
    agentSync: path.join(fixture.dir, ".workspace/state/AGENT_SYNC.md"),
    contextLedger: path.join(fixture.dir, ".workspace/state/CONTEXT_LEDGER.md"),
    loopState: path.join(fixture.dir, ".workspace/state/LOOP_STATE.md"),
    structureUpdateJson: path.join(fixture.dir, ".workspace/state/AUTO_HERMES_STRUCTURE_UPDATE.json"),
  });
