...
function backupFile(filePath) {
  const backupDir = resolvePath(".workspace/state/backups");
...
function checkReferentialIntegrity() {
  ...
  const syncContent = readTextFile(resolvePath(".workspace/state/AGENT_SYNC.md"));
  if (syncContent === null) {
...
function checkContextFreshness() {
  const findings = [];
  const ledgerPath = resolvePath(".workspace/state/CONTEXT_LEDGER.md");
...
export function runHealthCheck({ repair = false, write = false } = {}) {
  ...
  if (write) {
    const outputPath = resolvePath(".workspace/state/AUTO_HERMES_HEALTH_CHECK.json");
